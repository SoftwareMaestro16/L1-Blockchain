...
| Staking validators | `build\aetrad.exe query staking validators --grpc-addr $GRPC --grpc-insecure --node $NODE --output json` | `cosmos.staking.v1beta1.Query/Validators` | `GET /cosmos/staking/v1beta1/validators?pagination.limit=100` | `{"pagination":{"limit":"100"}}` | `{"validators":[{"operator_address":"aevaloper1...","status":"BOND_STATUS_BONDED"}]}` |
