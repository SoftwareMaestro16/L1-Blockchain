...
| Staking delegate | SDK requires delegator signature | malformed `AE...` validator address; wrong delegation denom | insufficient funds and invalid validator fail safely | `app/pos_test.go` and `tests/e2e/pos_smoke.ps1` cover signed replay/sequence | staking keeper direct validator/delegation keys |
