...
const (
	...
	DefaultOnChainAuthModel      = "aetheris-account"
...
func BuildModuleMsgServiceDescriptor(wrapper OnChainServiceWrapper) (ServiceDescriptor, error) {
	if wrapper.ModuleName == "" {
		return ServiceDescriptor{}, errors.New("aethercore module MsgServer service requires module name")
	}
	if wrapper.ContractAddress != "" {
		return ServiceDescriptor{}, errors.New("aethercore module MsgServer service must not define contract address")
	}
...
func BuildContractServiceDescriptor(wrapper OnChainServiceWrapper) (ServiceDescriptor, error) {
	if wrapper.ContractAddress == "" {
		return ServiceDescriptor{}, errors.New("aethercore contract service requires contract address")
	}
	if wrapper.ModuleName != "" {
		return ServiceDescriptor{}, errors.New("aethercore contract service must not define module name")
	}
	return BuildOnChainServiceDescriptor(wrapper)
}
...
func NewOnChainServiceReceipt(ctx ServiceConsensusContext, state CoreState, call ServiceCallEnvelope, result ExecutionResult, gasUsed uint64) (ServiceCallReceipt, error) {
	...
	if call.Kind != ServiceCallKindOnChain {
		return ServiceCallReceipt{}, errors.New("aethercore on-chain service receipt requires on-chain call kind")
	}
	descriptor, found := state.ServiceByID(call.ServiceID)
	if !found {
		return ServiceCallReceipt{}, fmt.Errorf("aethercore service %s is not registered", call.ServiceID)
	}
	if descriptor.ServiceType != ServiceTypeOnChain {
		return ServiceCallReceipt{}, errors.New("aethercore on-chain service receipt requires on-chain service")
	}
...
func NewServiceStateProofQuery(descriptor ServiceDescriptor, height uint64, keyHash string, proofRoot string) (ServiceStateProofQuery, error) {
	...
	if descriptor.ServiceType != ServiceTypeOnChain {
		return ServiceStateProofQuery{}, errors.New("aethercore service state proof query requires on-chain service")
	}
...
func (wrapper OnChainServiceWrapper) Validate() error {
	if err := validatePolicyID("aethercore on-chain service id", wrapper.ServiceID); err != nil {
		return err
	}
	if err := addressing.ValidateAuthorityAddress("aethercore on-chain service owner", wrapper.Owner); err != nil {
		return err
	}
	...
	if wrapper.ModuleName == "" && wrapper.ContractAddress == "" {
		return errors.New("aethercore on-chain service requires module name or contract address")
	}
	if wrapper.ModuleName != "" && wrapper.ContractAddress != "" {
		return errors.New("aethercore on-chain service must not define both module name and contract address")
	}
	if wrapper.ModuleName != "" {
	...
	if wrapper.ContractAddress != "" {
		if err := validatePolicyID("aethercore on-chain service contract address", wrapper.ContractAddress); err != nil {
			return err
		}
	}
	if err := validatePolicyID("aethercore on-chain service interface id", wrapper.InterfaceID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore on-chain service endpoint key", wrapper.EndpointKey); err != nil {
		return err
	}
	if wrapper.Version == 0 {
		return errors.New("aethercore on-chain service version must be positive")
	}
	if err := ValidateHash("aethercore on-chain service availability hash", wrapper.AvailabilityHash); err != nil {
		return err
	}
	if wrapper.StateRootType == "" {
		return errors.New("aethercore on-chain service requires state root type")
	}
	if err := validateToken("aethercore on-chain service state root type", string(wrapper.StateRootType), MaxScopeLength); err != nil {
		return err
	}
	if !IsServiceReceiptPolicy(wrapper.ReceiptPolicy) {
		return fmt.Errorf("unknown aethercore on-chain service receipt policy %q", wrapper.ReceiptPolicy)
	}
	if err := validatePolicyID("aethercore on-chain service payment denom", wrapper.PaymentDenom); err != nil {
		return err
	}
	if err := validateAmountString("aethercore on-chain service payment amount", wrapper.PaymentAmount); err != nil {
		return err
	}
	if wrapper.CreatedHeight == 0 {
		return errors.New("aethercore on-chain service created height must be positive")
	}
	if wrapper.UpdatedHeight < wrapper.CreatedHeight {
		return errors.New("aethercore on-chain service updated height must not precede created height")
	}
	if wrapper.ExpiryHeight != 0 && wrapper.ExpiryHeight <= wrapper.UpdatedHeight {
		return errors.New("aethercore on-chain service expiry height must exceed updated height")
	}
	if err := validateOptionalHash("aethercore on-chain service metadata hash", wrapper.MetadataHash); err != nil {
		return err
	}
...
func (method OnChainServiceMethod) Validate() error {
	if err := validatePolicyID("aethercore on-chain method id", method.MethodID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore on-chain method name", method.Name); err != nil {
		return err
	}
	if err := ValidateHash("aethercore on-chain method input schema hash", method.InputSchemaHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore on-chain method output schema hash", method.OutputSchemaHash); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore on-chain method gas model", method.GasModel); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore on-chain method payment model", method.RequiredPaymentModel); err != nil {
		return err
	}
	if !IsServiceFailureBehavior(method.FailurePolicy) {
		return fmt.Errorf("unknown aethercore on-chain method failure policy %q", method.FailurePolicy)
	}
	if method.TimeoutHeightDelta == 0 {
		return errors.New("aethercore on-chain method timeout must be positive")
	}
	return nil
}
...
func (query ServiceStateProofQuery) Validate() error {
	if err := validatePolicyID("aethercore service state proof service id", query.ServiceID); err != nil {
		return err
	}
	if query.Height == 0 {
		return errors.New("aethercore service state proof height must be positive")
	}
	if query.StateRootType == "" {
		return errors.New("aethercore service state proof requires state root type")
	}
	if err := validateToken("aethercore service state proof root type", string(query.StateRootType), MaxScopeLength); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service state proof key hash", query.KeyHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service state proof root", query.ProofRoot); err != nil {
		return err
	}
	if query.QueryHash != "" {
		if err := ValidateHash("aethercore service state proof query hash", query.QueryHash); err != nil {
			return err
		}
		if expected := ComputeServiceStateProofQueryHash(query); query.QueryHash != expected {
			return fmt.Errorf("aethercore service state proof query hash mismatch: expected %s", expected)
		}
	}
	return nil
}
...
func ComputeServiceStateProofQueryHash(query ServiceStateProofQuery) string {
	return hashParts(
		"aetheris-aek-service-state-proof-query-v1",
		query.ServiceID,
...
func validateOnChainMethods(methods []OnChainServiceMethod) error {
	if len(methods) == 0 {
		return errors.New("aethercore on-chain service requires method set")
	}
	var previous string
	...
	for i, method := range methods {
		...
		if _, found := seen[method.MethodID]; found {
			return fmt.Errorf("duplicate aethercore on-chain method %s", method.MethodID)
		}
		seen[method.MethodID] = struct{}{}
		if i > 0 && previous >= method.MethodID {
			return errors.New("aethercore on-chain methods must be sorted canonically")
		}
