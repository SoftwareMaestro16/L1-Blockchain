...
func DefaultNonGoalManifest() (NonGoalManifest, error) {
	...
	nonGoals := []NonGoal{
		...
		nonGoal(NonGoalNoUIAssumptions, "client interface scope", []CosmosSDKModuleName{CosmosModuleAetherCore, CosmosModuleServices, CosmosModuleZones}, false, "protocol state does not depend on client layout or UI flow", "queries expose data without presentation assumptions"),
...
func (manifest NonGoalManifest) ValidateFormat() error {
	...
	if len(manifest.NonGoals) != len(required) {
		return fmt.Errorf("aethercore non-goal manifest must include %d required non-goals", len(required))
	}
	...
	for i, nonGoal := range manifest.NonGoals {
		...
		if _, found := seen[nonGoal.ID]; found {
			return fmt.Errorf("duplicate aethercore non-goal %s", nonGoal.ID)
		}
		seen[nonGoal.ID] = struct{}{}
		if i > 0 && previous >= nonGoal.ID {
			return errors.New("aethercore non-goals must be sorted canonically")
		}
		previous = nonGoal.ID
	}
	for _, id := range required {
		if _, found := seen[NonGoalID(id)]; !found {
			return fmt.Errorf("missing aethercore non-goal %s", id)
		}
	}
	if manifest.ManifestHash != "" {
		return ValidateHash("aethercore non-goal manifest hash", manifest.ManifestHash)
	}
	return nil
}
...
func (manifest NonGoalManifest) Validate() error {
	...
	if manifest.ManifestHash != ComputeNonGoalManifestHash(manifest) {
		return errors.New("aethercore non-goal manifest hash mismatch")
	}
	return nil
}
...
func (nonGoal NonGoal) ValidateFormat() error {
	...
	if !IsRequiredNonGoalID(nonGoal.ID) {
		return fmt.Errorf("unknown aethercore non-goal %q", nonGoal.ID)
	}
	if err := validateRoadmapText("aethercore non-goal boundary", nonGoal.Boundary); err != nil {
		return err
	}
	if len(nonGoal.Modules) == 0 {
		return fmt.Errorf("aethercore non-goal %s must name at least one module", nonGoal.ID)
	}
	...
	for i, moduleName := range nonGoal.Modules {
		if !IsRequiredCosmosSDKModule(moduleName) {
			return fmt.Errorf("aethercore non-goal %s references unknown module %s", nonGoal.ID, moduleName)
		}
		if _, found := seenModules[moduleName]; found {
			return fmt.Errorf("duplicate aethercore non-goal module %s", moduleName)
		}
		seenModules[moduleName] = struct{}{}
		if i > 0 && previousModule >= moduleName {
			return fmt.Errorf("aethercore non-goal %s modules must be sorted canonically", nonGoal.ID)
		}
		previousModule = moduleName
	}
	if !nonGoal.Forbidden {
		return fmt.Errorf("aethercore non-goal %s must be forbidden", nonGoal.ID)
	}
	if !nonGoal.ConsensusBoundary {
		return fmt.Errorf("aethercore non-goal %s must define consensus boundary", nonGoal.ID)
	}
	if nonGoal.ID == NonGoalNoCanonicalOffchainResultWithoutProof && !nonGoal.CommitmentRequired {
		return fmt.Errorf("aethercore non-goal %s must require committed proof or receipt", nonGoal.ID)
	}
	if err := validateNonGoalRationale(nonGoal.ID, nonGoal.Rationale); err != nil {
		return err
	}
	if nonGoal.NonGoalHash != "" {
		return ValidateHash("aethercore non-goal hash", nonGoal.NonGoalHash)
	}
	return nil
}
...
func (nonGoal NonGoal) Validate() error {
	...
	if nonGoal.NonGoalHash != ComputeNonGoalHash(nonGoal) {
		return errors.New("aethercore non-goal hash mismatch")
	}
	return nil
}
...
func ComputeNonGoalHash(nonGoal NonGoal) string {
	nonGoal = normalizeNonGoal(nonGoal)
	return hashRoot("aetheris-aek-non-goal-v1", func(w byteWriter) {
...
func ComputeNonGoalManifestHash(manifest NonGoalManifest) string {
	manifest.NonGoals = normalizeNonGoals(manifest.NonGoals)
	return hashRoot("aetheris-aek-non-goal-manifest-v1", func(w byteWriter) {
...
func validateNonGoalRationale(id NonGoalID, rationale []string) error {
	if len(rationale) == 0 {
		return fmt.Errorf("aethercore non-goal %s rationale is required", id)
	}
	seen := make(map[string]struct{}, len(rationale))
	var previous string
	for i, item := range rationale {
		if err := validateRoadmapText("aethercore non-goal rationale", item); err != nil {
			return err
		}
		if _, found := seen[item]; found {
			return fmt.Errorf("duplicate aethercore non-goal rationale %s", item)
		}
		seen[item] = struct{}{}
		if i > 0 && previous >= item {
			return fmt.Errorf("aethercore non-goal %s rationale must be sorted canonically", id)
		}
		previous = item
	}
	return nil
