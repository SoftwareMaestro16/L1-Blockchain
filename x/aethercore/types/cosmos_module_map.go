...
func CosmosSDKNewModules() []CosmosModuleDescriptor {
	return []CosmosModuleDescriptor{
		cosmosModule("x/aethercore", "Zone registry, commitments, routing epochs, proof roots.", "Core", []string{"Store v2", "ABCI++", "CometBFT finality"}, "Replays identical zone commitments and global roots from reordered inputs."),
		cosmosModule("x/msgbus", "First-class messages, inbox/outbox, receipts, routing.", "Core + zones", []string{"x/aethercore", "x/shards", "x/proofregistry"}, "Proves message inclusion and receipt delivery across zones and shards."),
		cosmosModule("x/zones", "Zone lifecycle, zone execution adapter, zone params.", "Core", []string{"x/aethercore"}, "Every zone exposes deterministic execute, inbound, export, and import hooks."),
		cosmosModule("x/shards", "Shard layout, split/merge, shard metrics.", "Zones", []string{"x/aethercore", "Store v2", "BlockSTM"}, "Routing is stable inside an epoch and changes only at committed layout boundaries."),
		cosmosModule("x/proofregistry", "Universal proof root registry and proof queries.", "Core", []string{"Store v2", "x/aethercore"}, "Clients verify account, message, zone, shard, identity, contract, and payment proofs."),
		...
		cosmosModule("x/zonefees", "Zone-local fee accounting and aggregation.", "Core + zones", []string{"x/aethercore", "x/payments"}, "Per-shard fees aggregate into zone and global fee roots."),
		cosmosModule("x/zonemempool", "Mempool classification and transaction grouping.", "Node-side + core checks", []string{"x/aethercore", "x/shards"}, "Proposal grouping is deterministic and validators reject malformed schedules."),
	}
}
...
func (m CosmosModuleMap) ValidateFormat() error {
	...
	if m.Version != CosmosModuleMapVersion {
		return fmt.Errorf("aethercore cosmos module map version must be %d", CosmosModuleMapVersion)
	}
	if len(m.Modules) == 0 {
		return errors.New("aethercore cosmos module map requires modules")
	}
	...
	for _, module := range m.Modules {
		...
		if _, found := seen[module.Module]; found {
			return fmt.Errorf("aethercore duplicate cosmos module %s", module.Module)
		}
		seen[module.Module] = struct{}{}
		if previous != "" && previous >= module.Module {
			return errors.New("aethercore cosmos module map must be sorted by module")
		}
		previous = module.Module
	}
	if m.Root != "" {
		if err := ValidateHash("aethercore cosmos module map root", m.Root); err != nil {
			return err
		}
	}
	return nil
...
func (m CosmosModuleMap) Validate() error {
	...
	if m.Root == "" {
		return errors.New("aethercore cosmos module map root is required")
	}
	expected := ComputeCosmosModuleMapRoot(m.Modules)
	if m.Root != expected {
		return fmt.Errorf("aethercore cosmos module map root mismatch: expected %s", expected)
	}
	return nil
}
...
func BuildCosmosModuleDescriptor(desc CosmosModuleDescriptor) (CosmosModuleDescriptor, error) {
	...
	if desc.DescriptorHash != "" {
		return CosmosModuleDescriptor{}, errors.New("aethercore cosmos module descriptor hash must be empty before construction")
	}
...
func (desc CosmosModuleDescriptor) ValidateFormat() error {
	...
	if !strings.HasPrefix(desc.Module, "x/") {
		return errors.New("aethercore cosmos module name must start with x/")
	}
	if err := validateToken("aethercore cosmos module name", desc.Module, MaxScopeLength); err != nil {
		return err
	}
	if desc.Responsibility == "" {
		return errors.New("aethercore cosmos module responsibility is required")
	}
	if desc.Zone == "" {
		return errors.New("aethercore cosmos module zone is required")
	}
	if len(desc.Dependencies) == 0 {
		return errors.New("aethercore cosmos module dependencies are required")
	}
	if desc.AcceptanceSignal == "" {
		return errors.New("aethercore cosmos module acceptance signal is required")
	}
	seenDeps := make(map[string]struct{}, len(desc.Dependencies))
	previous := ""
	for _, dep := range desc.Dependencies {
		if dep == "" {
			return errors.New("aethercore cosmos module dependency is required")
		}
		if _, found := seenDeps[dep]; found {
			return fmt.Errorf("aethercore duplicate cosmos module dependency %s", dep)
		}
		seenDeps[dep] = struct{}{}
		if previous != "" && previous >= dep {
			return errors.New("aethercore cosmos module dependencies must be sorted")
		}
		previous = dep
	}
	if desc.DescriptorHash != "" {
		if err := ValidateHash("aethercore cosmos module descriptor hash", desc.DescriptorHash); err != nil {
			return err
		}
	}
	return nil
...
func (desc CosmosModuleDescriptor) Validate() error {
	...
	if desc.DescriptorHash == "" {
		return errors.New("aethercore cosmos module descriptor hash is required")
	}
	expected := ComputeCosmosModuleDescriptorHash(desc)
	if desc.DescriptorHash != expected {
		return fmt.Errorf("aethercore cosmos module descriptor hash mismatch: expected %s", expected)
	}
	return nil
}
...
func ComputeCosmosModuleDescriptorHash(desc CosmosModuleDescriptor) string {
	desc = desc.Normalize()
	parts := []string{
		"aetheris-cosmos-module-descriptor-v1",
		desc.Module,
		desc.Responsibility,
		desc.Zone,
...
func ComputeCosmosModuleMapRoot(modules []CosmosModuleDescriptor) string {
	normalized := normalizeCosmosModuleDescriptors(modules)
	parts := []string{"aetheris-cosmos-module-map-v1", fmt.Sprintf("%020d", CosmosModuleMapVersion)}
...
func ValidateCosmosSDKNewModuleMap() error {
	...
	for _, moduleName := range required {
		if _, found := byModule[moduleName]; !found {
			return fmt.Errorf("aethercore cosmos module map missing %s", moduleName)
		}
	}
	return nil
}
