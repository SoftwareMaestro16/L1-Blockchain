...
func ProcessKernelProposal(ctx KernelConsensusContext, state CoreState, plan KernelBlockPlan) error {
	...
	if plan.Phase != KernelPhasePrepareProposal && plan.Phase != KernelPhaseProcessProposal {
		return errors.New("aethercore kernel proposal is not in a processable phase")
	}
	return nil
}
...
func FinalizeKernelBlock(ctx KernelConsensusContext, state CoreState, plan KernelBlockPlan, input KernelFinalizationInput) (CoreState, KernelFinalization, error) {
	...
	if input.Contributions.ReceiptsRoot != receiptsRoot {
		return CoreState{}, KernelFinalization{}, errors.New("aethercore kernel receipts root contribution mismatch")
	}
	next := state.Export()
	...
	for _, commitment := range commitments {
		if commitment.Height != ctx.Height {
			return CoreState{}, KernelFinalization{}, errors.New("aethercore kernel commitment height mismatch")
		}
	...
	if !found {
		return CoreState{}, KernelFinalization{}, errors.New("aethercore kernel global root was not committed")
	}
...
func ValidateKernelBlockPlanForState(ctx KernelConsensusContext, state CoreState, plan KernelBlockPlan) error {
	...
	if plan.Height != ctx.Height || plan.ChainID != ctx.ChainID {
		return errors.New("aethercore kernel plan consensus context mismatch")
	}
	if plan.Phase != KernelPhasePrepareProposal && plan.Phase != KernelPhaseProcessProposal {
		return fmt.Errorf("unknown aethercore kernel proposal phase %q", plan.Phase)
	}
	if err := ValidateHash("aethercore kernel proposal root", plan.ProposalRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore kernel routing root", plan.RoutingRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore kernel previous global root", plan.PreviousGlobalRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore kernel params hash", plan.ParamsHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore kernel plan hash", plan.PlanHash); err != nil {
		return err
	}
	...
	if proposalRoot != plan.ProposalRoot {
		return errors.New("aethercore kernel proposal root mismatch")
	}
	routingRoot := EmptyRootHash
	if table, found := state.LatestRoutingTableAtHeight(ctx.Height); found {
	...
	if routingRoot != plan.RoutingRoot {
		return errors.New("aethercore kernel routing root mismatch")
	}
	...
	if previousRoot != plan.PreviousGlobalRoot {
		return errors.New("aethercore kernel previous global root mismatch")
	}
	if paramsHash := ComputeAetherCoreParamsHash(state.Params); paramsHash != plan.ParamsHash {
		return errors.New("aethercore kernel params hash mismatch")
	}
	if expected := ComputeKernelBlockPlanHash(plan); expected != plan.PlanHash {
		return fmt.Errorf("aethercore kernel plan hash mismatch: expected %s", expected)
	}
	return nil
}

func (ctx KernelConsensusContext) Validate() error {
	if err := validatePolicyID("aethercore kernel chain id", ctx.ChainID); err != nil {
		return err
	}
	if ctx.Height == 0 {
		return errors.New("aethercore kernel consensus height must be positive")
	}
	if ctx.BlockTimeUnix < 0 {
		return errors.New("aethercore kernel block time must be consensus supplied")
	}
	return nil
}
...
func NewKernelBlockHeaderCommitment(ctx KernelConsensusContext, previousAppHash string, root GlobalStateRoot, snapshot RootSnapshot) (KernelBlockHeaderCommitment, error) {
	...
	if err := ValidateHash("aethercore kernel previous app hash", previousAppHash); err != nil {
		return KernelBlockHeaderCommitment{}, err
	}
	...
	if root.Height != ctx.Height || snapshot.Height != ctx.Height {
		return KernelBlockHeaderCommitment{}, errors.New("aethercore kernel header height mismatch")
	}
	header := KernelBlockHeaderCommitment{
		Height:          ctx.Height,
...
func (h KernelBlockHeaderCommitment) Validate() error {
	if h.Height == 0 {
		return errors.New("aethercore kernel header height must be positive")
	}
	if h.TimeUnix < 0 {
		return errors.New("aethercore kernel header time must be consensus supplied")
	}
	if err := ValidateHash("aethercore kernel header previous app hash", h.PreviousAppHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore kernel header zones root", h.ZonesRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore kernel header messages root", h.MessagesRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore kernel header receipts root", h.ReceiptsRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore kernel header hash", h.HeaderHash); err != nil {
		return err
	}
	if expected := ComputeKernelBlockHeaderHash(h); expected != h.HeaderHash {
		return fmt.Errorf("aethercore kernel header hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (f KernelFinalization) Validate() error {
	if f.Height == 0 {
		return errors.New("aethercore kernel finalization height must be positive")
	}
	if err := validatePolicyID("aethercore kernel finalization chain id", f.ChainID); err != nil {
		return err
	}
	if f.Phase != KernelPhaseFinalizeBlock {
		return fmt.Errorf("unknown aethercore kernel finalization phase %q", f.Phase)
	}
	if err := ValidateHash("aethercore kernel finalization plan hash", f.PlanHash); err != nil {
		return err
	}
	...
	if f.GlobalRoot.Height != f.Height || f.RootSnapshot.Height != f.Height {
		return errors.New("aethercore kernel finalization height mismatch")
	}
	if f.Header.Height != f.Height {
		return errors.New("aethercore kernel finalization header height mismatch")
	}
	if f.Header.ZonesRoot != f.GlobalRoot.ZonesRoot {
		return errors.New("aethercore kernel header zones root mismatch")
	}
	if f.Header.MessagesRoot != f.RootSnapshot.Finality.GlobalMessageRoot {
		return errors.New("aethercore kernel header messages root mismatch")
	}
	if f.Header.ReceiptsRoot != f.RootSnapshot.Finality.ExecutionReceiptRoot {
		return errors.New("aethercore kernel header receipts root mismatch")
	}
	if f.RootSnapshot.Finality.GlobalStateRoot != f.GlobalRoot.GlobalRoot {
		return errors.New("aethercore kernel finality global root mismatch")
	}
	if f.RootSnapshot.Finality.ExecutionReceiptRoot != f.ReceiptsRoot {
		return errors.New("aethercore kernel finality receipt root mismatch")
	}
	if f.GlobalRoot.ReceiptsRoot != f.ReceiptsRoot {
		return errors.New("aethercore kernel global receipt root mismatch")
	}
	if err := ValidateHash("aethercore kernel finalization receipts root", f.ReceiptsRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore kernel finality hash", f.FinalityHash); err != nil {
		return err
	}
	if expected := ComputeKernelFinalizationHash(f); expected != f.FinalityHash {
		return fmt.Errorf("aethercore kernel finality hash mismatch: expected %s", expected)
	}
	return nil
}
...
func ComputeProposalScheduleRoot(schedule ProposalSchedule) (string, error) {
	...
	parts := []string{"aetheris-aek-proposal-schedule-root-v1", fmt.Sprint(schedule.Height), fmt.Sprint(len(groups))}
...
func ComputeExecutionReceiptsRoot(receipts []ExecutionReceipt) (string, error) {
	...
	parts := []string{"aetheris-aek-execution-receipts-root-v1", fmt.Sprint(len(ordered))}
	...
	for i, receipt := range ordered {
		...
		if _, found := seen[receipt.ReceiptHash]; found {
			return "", errors.New("duplicate aethercore execution receipt")
		}
		seen[receipt.ReceiptHash] = struct{}{}
		if i > 0 && compareExecutionReceipts(previous, receipt) >= 0 {
			return "", errors.New("aethercore execution receipts must be sorted canonically")
		}
...
func ComputeAetherCoreParamsHash(params AetherCoreParams) string {
	return hashParts(
		"aetheris-aek-params-v1",
...
func ComputeKernelBlockPlanHash(plan KernelBlockPlan) string {
	return hashParts(
		"aetheris-aek-kernel-block-plan-v1",
		plan.ChainID,
...
func ComputeKernelFinalizationHash(finalization KernelFinalization) string {
	return hashParts(
		"aetheris-aek-kernel-finalization-v1",
...
func BuildKernelExportManifest(state CoreState, height uint64, appHash string) (ExportManifest, error) {
	...
	if !found {
		return ExportManifest{}, fmt.Errorf("aethercore kernel export missing global root at height %d", height)
	}
...
func ValidateKernelImport(state CoreState, manifest ExportManifest) error {
	...
	if !found {
		return fmt.Errorf("aethercore kernel import missing global root at height %d", manifest.Height)
	}
	...
	if manifest.ServiceDescriptorCount != uint64(len(state.ServiceDescriptors)) {
		return errors.New("aethercore kernel import service descriptor count mismatch")
	}
	return nil
}
