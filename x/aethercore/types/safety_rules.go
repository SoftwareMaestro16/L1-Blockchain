...
func (s SafetyRulesSpec) ValidateFormat() error {
	...
	if s.Version != SafetyRulesSpecVersion {
		return fmt.Errorf("aethercore safety rules spec version must be %d", SafetyRulesSpecVersion)
	}
	if len(s.Determinism) == 0 {
		return errors.New("aethercore safety rules spec requires determinism rules")
	}
	if len(s.RoutingSafety) == 0 {
		return errors.New("aethercore safety rules spec requires routing safety rules")
	}
...
func (s SafetyRulesSpec) Validate() error {
	...
	if err := ValidateHash("aethercore determinism safety root", s.DeterminismRoot); err != nil {
		return err
	}
	if s.DeterminismRoot != ComputeDeterminismRulesRoot(s.Determinism) {
		return errors.New("aethercore determinism safety root mismatch")
	}
	if err := ValidateHash("aethercore routing safety root", s.RoutingRoot); err != nil {
		return err
	}
	if s.RoutingRoot != ComputeRoutingSafetyRulesRoot(s.RoutingSafety) {
		return errors.New("aethercore routing safety root mismatch")
	}
	if err := ValidateHash("aethercore safety rules root", s.Root); err != nil {
		return err
	}
	if s.Root != ComputeSafetyRulesSpecRoot(s) {
		return errors.New("aethercore safety rules root mismatch")
	}
	return nil
}
...
func (r DeterminismRule) Validate() error {
	...
	if !IsDeterminismRuleID(r.RuleID) {
		return fmt.Errorf("unknown aethercore determinism rule %q", r.RuleID)
	}
	if r.Rule == "" || r.Enforcement == "" || r.Evidence == "" {
		return errors.New("aethercore determinism rule requires rule, enforcement, and evidence")
	}
	if err := ValidateHash("aethercore determinism rule hash", r.DescriptorHash); err != nil {
		return err
	}
	if r.DescriptorHash != ComputeDeterminismRuleHash(r) {
		return errors.New("aethercore determinism rule hash mismatch")
	}
	return nil
}
...
func (r RoutingSafetyRule) Validate() error {
	...
	if !IsRoutingSafetyRuleID(r.RuleID) {
		return fmt.Errorf("unknown aethercore routing safety rule %q", r.RuleID)
	}
	if r.Rule == "" || r.Enforcement == "" || r.Evidence == "" {
		return errors.New("aethercore routing safety rule requires rule, enforcement, and evidence")
	}
	if err := ValidateHash("aethercore routing safety rule hash", r.DescriptorHash); err != nil {
		return err
	}
	if r.DescriptorHash != ComputeRoutingSafetyRuleHash(r) {
		return errors.New("aethercore routing safety rule hash mismatch")
	}
	return nil
}
...
func ValidateConsensusDeterminismEvidence(e ConsensusDeterminismEvidence) error {
	...
	if e.ExternalAPICalls {
		return errors.New("aethercore determinism violation: external API calls")
	}
	if e.LocalClockUsage {
		return errors.New("aethercore determinism violation: local clock usage")
	}
	if e.RandomShardPlacement {
		return errors.New("aethercore determinism violation: random shard placement")
	}
	if e.MapIterationOutput {
		return errors.New("aethercore determinism violation: nondeterministic map iteration")
	}
	if e.MempoolOnlyResult {
		return errors.New("aethercore determinism violation: mempool-only execution result")
	}
	if e.FloatingPointMath {
		return errors.New("aethercore determinism violation: floating point consensus math")
	}
	if err := ValidateHash("aethercore determinism evidence hash", e.EvidenceHash); err != nil {
		return err
	}
	if e.EvidenceHash != ComputeConsensusDeterminismEvidenceHash(e) {
		return errors.New("aethercore determinism evidence hash mismatch")
	}
	return nil
}
...
func ValidateRoutingSafetyEvidence(e RoutingSafetyEvidence) error {
	...
	for _, item := range []struct {
		...
		{"aethercore routing safety table hash", e.RoutingTableHash},
		{"aethercore routing safety metrics root", e.RoutingMetricsRoot},
		{"aethercore routing safety tie-break hash", e.DeterministicTieBreakHash},
		{"aethercore routing safety failure receipt root", e.FailureReceiptRoot},
		{"aethercore routing safety bounce cap hash", e.BounceValueCapHash},
		{"aethercore routing safety evidence hash", e.EvidenceHash},
	} {
	...
	if e.FailedRouteValueNAET > 0 && e.ReceiptedValueNAET == 0 {
		return errors.New("aethercore routing safety violation: failed route value requires receipt")
	}
	if e.ReceiptedValueNAET > e.FailedRouteValueNAET {
		return errors.New("aethercore routing safety violation: receipt value exceeds failed route value")
	}
	if e.BounceValueNAET > e.OriginalBounceValueNAET {
		return errors.New("aethercore routing safety violation: bounce creates extra value")
	}
	if e.EvidenceHash != ComputeRoutingSafetyEvidenceHash(e) {
		return errors.New("aethercore routing safety evidence hash mismatch")
	}
	return nil
}
...
func ValidateSafetyRulesCoverage() error {
	...
	for _, ruleID := range []DeterminismRuleID{
		...
		if _, found := determinism[ruleID]; !found {
			return fmt.Errorf("aethercore safety spec missing determinism rule %s", ruleID)
		}
	}
	...
	for _, ruleID := range []RoutingSafetyRuleID{
		...
		if _, found := routing[ruleID]; !found {
			return fmt.Errorf("aethercore safety spec missing routing rule %s", ruleID)
		}
	}
	return nil
}
...
func ComputeDeterminismRuleHash(rule DeterminismRule) string {
	rule = rule.Normalize()
	return hashParts("aetheris-determinism-rule-v1", string(rule.RuleID), rule.Rule, rule.Enforcement, rule.Evidence)
}

func ComputeRoutingSafetyRuleHash(rule RoutingSafetyRule) string {
	rule = rule.Normalize()
	return hashParts("aetheris-routing-safety-rule-v1", string(rule.RuleID), rule.Rule, rule.Enforcement, rule.Evidence)
}

func ComputeDeterminismRulesRoot(rules []DeterminismRule) string {
	ordered := normalizeDeterminismRules(rules)
	parts := []string{"aetheris-determinism-rules-root-v1", fmt.Sprintf("%020d", SafetyRulesSpecVersion)}
...
func ComputeRoutingSafetyRulesRoot(rules []RoutingSafetyRule) string {
	...
	parts := []string{"aetheris-routing-safety-rules-root-v1", fmt.Sprintf("%020d", SafetyRulesSpecVersion)}
	for _, rule := range ordered {
		parts = append(parts, string(rule.RuleID), rule.DescriptorHash)
	}
	return hashParts(parts...)
...
func ComputeSafetyRulesSpecRoot(spec SafetyRulesSpec) string {
	spec = spec.Normalize()
	return hashParts("aetheris-safety-rules-spec-v1", fmt.Sprintf("%020d", spec.Version), spec.DeterminismRoot, spec.RoutingRoot)
}

func ComputeConsensusDeterminismEvidenceHash(e ConsensusDeterminismEvidence) string {
	e.EvidenceHash = ""
	return hashParts(
		"aetheris-determinism-evidence-v1",
...
func ComputeRoutingSafetyEvidenceHash(e RoutingSafetyEvidence) string {
	...
	e.EvidenceHash = ""
	return hashParts(
		"aetheris-routing-safety-evidence-v1",
		e.RoutingTableHash,
...
func validateDeterminismRules(rules []DeterminismRule) error {
	...
	for i, rule := range rules {
		...
		if _, found := seen[rule.RuleID]; found {
			return fmt.Errorf("duplicate aethercore determinism rule %s", rule.RuleID)
		}
		seen[rule.RuleID] = struct{}{}
		if i > 0 && previous >= rule.RuleID {
			return errors.New("aethercore determinism rules must be sorted canonically")
		}
...
func validateRoutingSafetyRules(rules []RoutingSafetyRule) error {
	...
	for i, rule := range rules {
		...
		if _, found := seen[rule.RuleID]; found {
			return fmt.Errorf("duplicate aethercore routing safety rule %s", rule.RuleID)
		}
		seen[rule.RuleID] = struct{}{}
		if i > 0 && previous >= rule.RuleID {
			return errors.New("aethercore routing safety rules must be sorted canonically")
		}
		previous = rule.RuleID
	}
	return nil
