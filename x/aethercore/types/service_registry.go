...
func NewServiceRegistry(mode ServiceRegistryMode, descriptors []ServiceDescriptor, height uint64) (ServiceRegistry, error) {
	if !IsServiceRegistryMode(mode) {
		return ServiceRegistry{}, fmt.Errorf("unknown aethercore service registry mode %q", mode)
	}
	if height == 0 {
		return ServiceRegistry{}, errors.New("aethercore service registry height must be positive")
	}
...
func RegisterServiceRegistryRecord(registry ServiceRegistry, record ServiceRegistryRecord, height uint64) (ServiceRegistry, error) {
	...
	if height < registry.UpdatedHeight {
		return ServiceRegistry{}, errors.New("aethercore service registry update height must not go backwards")
	}
	...
	if _, found := registry.RecordByID(record.ServiceID); found {
		return ServiceRegistry{}, fmt.Errorf("aethercore service registry record %s already exists", record.ServiceID)
	}
	next := registry.clone()
	...
	for i := range next.Records {
		if next.Records[i].ServiceID == record.ServiceID {
			if record.Version < next.Records[i].Version {
				return ServiceRegistry{}, errors.New("aethercore service registry version must not decrease")
			}
			next.Records[i] = record
			return finalizeServiceRegistry(next, height)
		}
	}
	return ServiceRegistry{}, fmt.Errorf("aethercore service registry record %s not found", record.ServiceID)
}
...
func RenewServiceRegistryRecord(registry ServiceRegistry, serviceID string, expiryHeight uint64, updatedHeight uint64) (ServiceRegistry, ServiceRegistryRecord, error) {
	...
	if !found {
		return ServiceRegistry{}, ServiceRegistryRecord{}, fmt.Errorf("aethercore service registry record %s not found", serviceID)
	}
	if expiryHeight <= record.ExpiryHeight {
		return ServiceRegistry{}, ServiceRegistryRecord{}, errors.New("aethercore service registry renewal must extend expiry")
	}
	if updatedHeight < record.UpdatedHeight {
		return ServiceRegistry{}, ServiceRegistryRecord{}, errors.New("aethercore service registry renewal height must not go backwards")
	}
...
func ExpireServiceRegistryRecords(registry ServiceRegistry, height uint64) (ServiceRegistry, []string, error) {
	...
	if height == 0 || height < registry.UpdatedHeight {
		return ServiceRegistry{}, nil, errors.New("aethercore service registry expiry height is invalid")
	}
	next := registry.clone()
...
func (record ServiceRegistryRecord) Validate() error {
	record = CanonicalServiceRegistryRecord(record)
	if err := validatePolicyID("aethercore service registry service id", record.ServiceID); err != nil {
		return err
	}
	if err := addressing.ValidateAuthorityAddress("aethercore service registry owner", record.Owner); err != nil {
		return err
	}
	if !IsServiceType(record.ServiceType) {
		return fmt.Errorf("unknown aethercore service registry type %q", record.ServiceType)
	}
	if err := ValidateHash("aethercore service registry interface hash", record.InterfaceHash); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore service registry endpoint", record.Endpoint); err != nil {
		return err
	}
	if record.ExecutionMode != ExecutionModeSync && record.ExecutionMode != ExecutionModeAsync {
		return fmt.Errorf("unknown aethercore service registry execution mode %q", record.ExecutionMode)
	}
	if err := validatePolicyID("aethercore service registry payment model", record.PaymentModel); err != nil {
		return err
	}
	if err := validateAmountString("aethercore service registry stake", record.Stake); err != nil {
		return err
	}
	if err := validateAmountString("aethercore service registry collateral", record.Collateral); err != nil {
		return err
	}
	if record.ServiceName != "" {
		if err := validatePolicyID("aethercore service registry service name", record.ServiceName); err != nil {
			return err
		}
	}
	if record.IdentityName != "" {
		if err := validatePolicyID("aethercore service registry identity name", record.IdentityName); err != nil {
			return err
		}
	}
	if record.ZoneID != "" {
	...
	if record.ContractAddress != "" {
		if err := validatePolicyID("aethercore service registry contract address", record.ContractAddress); err != nil {
			return err
		}
	}
	...
	if record.ProviderSet != "" {
		if err := validatePolicyID("aethercore service registry provider set", record.ProviderSet); err != nil {
			return err
		}
	}
	if !IsServiceVerificationModel(record.VerificationModel) {
		return fmt.Errorf("unknown aethercore service registry verification model %q", record.VerificationModel)
	}
	if !IsServiceTrustModel(record.TrustModel) {
		return fmt.Errorf("unknown aethercore service registry trust model %q", record.TrustModel)
	}
	if !IsServiceStatus(record.Status) {
		return fmt.Errorf("unknown aethercore service registry status %q", record.Status)
	}
	if record.Version == 0 {
		return errors.New("aethercore service registry version must be positive")
	}
	if err := ValidateHash("aethercore service registry descriptor hash", record.DescriptorHash); err != nil {
		return err
	}
	if err := validateOptionalHash("aethercore service registry metadata hash", record.MetadataHash); err != nil {
		return err
	}
	if record.CreatedHeight == 0 || record.UpdatedHeight < record.CreatedHeight {
		return errors.New("aethercore service registry heights are invalid")
	}
	if record.ExpiryHeight != 0 && record.ExpiryHeight <= record.UpdatedHeight && record.Status == ServiceStatusActive {
		return errors.New("aethercore active service registry record must not be expired")
	}
	if err := ValidateHash("aethercore service registry record hash", record.RecordHash); err != nil {
		return err
	}
	if expected := ComputeServiceRegistryRecordHash(record); record.RecordHash != expected {
		return fmt.Errorf("aethercore service registry record hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (registry ServiceRegistry) Validate() error {
	if !IsServiceRegistryMode(registry.Mode) {
		return fmt.Errorf("unknown aethercore service registry mode %q", registry.Mode)
	}
	if registry.UpdatedHeight == 0 {
		return errors.New("aethercore service registry updated height must be positive")
	}
	...
	if ComputeServiceInterfaceVersionIndexHash(index) != ComputeServiceInterfaceVersionIndexHash(registry.InterfaceIndex) {
		return errors.New("aethercore service registry interface index mismatch")
	}
	if err := ValidateHash("aethercore service registry root", registry.RegistryRoot); err != nil {
		return err
	}
	if expected := ComputeServiceRegistryRoot(registry); registry.RegistryRoot != expected {
		return fmt.Errorf("aethercore service registry root mismatch: expected %s", expected)
	}
	return nil
}
...
func (proof ServiceRegistryProof) ValidateForRegistry(registry ServiceRegistry) error {
	...
	if !found {
		return fmt.Errorf("aethercore service registry proof references unknown service %s", proof.ServiceID)
	}
	if proof.RegistryMode != registry.Mode || proof.RegistryRoot != registry.RegistryRoot ||
		...
		return errors.New("aethercore service registry proof does not match registry")
	}
	return proof.Validate()
}

func (proof ServiceRegistryProof) Validate() error {
	if err := validatePolicyID("aethercore service registry proof service id", proof.ServiceID); err != nil {
		return err
	}
	if !IsServiceRegistryMode(proof.RegistryMode) {
		return fmt.Errorf("unknown aethercore service registry proof mode %q", proof.RegistryMode)
	}
	if err := ValidateHash("aethercore service registry proof root", proof.RegistryRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service registry proof record hash", proof.RecordHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service registry proof descriptor hash", proof.DescriptorHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore service registry proof interface hash", proof.InterfaceHash); err != nil {
		return err
	}
	if proof.ProofHeight == 0 {
		return errors.New("aethercore service registry proof height must be positive")
	}
	if err := ValidateHash("aethercore service registry proof hash", proof.ProofHash); err != nil {
		return err
	}
	if expected := ComputeServiceRegistryProofHash(proof); proof.ProofHash != expected {
		return fmt.Errorf("aethercore service registry proof hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (record ServiceInterfaceVersionRecord) Validate() error {
	if err := ValidateHash("aethercore service interface version id", record.InterfaceID); err != nil {
		return err
	}
	if record.Version == 0 {
		return errors.New("aethercore service interface version must be positive")
	}
	if err := ValidateHash("aethercore service interface version hash", record.InterfaceHash); err != nil {
		return err
	}
	if err := validateSortedStringSet("aethercore service interface version service", record.ServiceIDs); err != nil {
		return err
	}
	if len(record.ServiceIDs) == 0 {
		return errors.New("aethercore service interface version requires services")
	}
	if err := validateOptionalHash("aethercore service interface version metadata", record.MetadataHash); err != nil {
		return err
	}
	if record.CreatedHeight == 0 {
		return errors.New("aethercore service interface version created height must be positive")
	}
	if err := ValidateHash("aethercore service interface version record hash", record.RecordHash); err != nil {
		return err
	}
	if expected := ComputeServiceInterfaceVersionRecordHash(record); record.RecordHash != expected {
		return fmt.Errorf("aethercore service interface version hash mismatch: expected %s", expected)
	}
	return nil
}
...
func ComputeServiceRegistryRecordHash(record ServiceRegistryRecord) string {
	record.RecordHash = ""
	return hashParts(
		"aetheris-aek-service-registry-record-v1",
		record.ServiceID,
		record.Owner,
...
func ComputeServiceRegistryRoot(registry ServiceRegistry) string {
	...
	parts := []string{
		"aetheris-aek-service-registry-root-v1",
...
func ComputeServiceRegistryProofHash(proof ServiceRegistryProof) string {
	return hashParts(
		"aetheris-aek-service-registry-proof-v1",
		proof.ServiceID,
...
func ComputeServiceInterfaceVersionRecordHash(record ServiceInterfaceVersionRecord) string {
	parts := []string{
		"aetheris-aek-service-interface-version-v1",
		record.InterfaceID,
		fmt.Sprint(record.Version),
		record.InterfaceHash,
		record.MetadataHash,
...
func ComputeServiceInterfaceVersionIndexHash(records []ServiceInterfaceVersionRecord) string {
	...
	parts := []string{"aetheris-aek-service-interface-index-v1", fmt.Sprint(len(ordered))}
...
func ComputeServicePaymentDiscoveryHash(discovery ServicePaymentDiscovery) string {
	return hashParts(
		"aetheris-aek-service-payment-discovery-v1",
		discovery.ServiceID,
...
func ComputeServiceTrustDiscoveryHash(discovery ServiceTrustDiscovery) string {
	return hashParts(
		"aetheris-aek-service-trust-discovery-v1",
		discovery.ServiceID,
...
func validateServiceRegistryRecords(records []ServiceRegistryRecord) error {
	...
	for _, record := range records {
		...
		if _, found := seen[record.ServiceID]; found {
			return fmt.Errorf("duplicate aethercore service registry record %s", record.ServiceID)
		}
		seen[record.ServiceID] = struct{}{}
		if previous != "" && previous >= record.ServiceID {
			return errors.New("aethercore service registry records must be sorted canonically")
		}
...
func validateServiceInterfaceVersionRecords(records []ServiceInterfaceVersionRecord) error {
	...
	for _, record := range records {
		...
		if previous != "" && previous >= key {
			return errors.New("aethercore service interface version records must be sorted canonically")
		}
		previous = key
	}
	return nil
