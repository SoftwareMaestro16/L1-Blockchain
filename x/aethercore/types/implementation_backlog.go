...
const (
	...
	BacklogItemAetherCoreSkeleton             BacklogItemID = "aethercore-skeleton"
...
func HighPriorityBacklogItems() []BacklogItem {
	return []BacklogItem{
		backlogItem(BacklogPriorityHigh, BacklogItemZoneDescriptorCommitmentLayout, "Define ZoneDescriptor, ZoneCommitment, and ShardLayout.", "x/aethercore types", []string{"zone descriptor validation", "zone commitment hash", "shard layout hash", "canonical export"}),
		backlogItem(BacklogPriorityHigh, BacklogItemAetherCoreSkeleton, "Implement x/aethercore skeleton.", "x/aethercore module", []string{"module name", "keeper shell", "params", "genesis import export"}),
...
func (s ImplementationBacklogSpec) ValidateFormat() error {
	...
	if s.Version != ImplementationBacklogSpecVersion {
		return fmt.Errorf("aethercore implementation backlog spec version must be %d", ImplementationBacklogSpecVersion)
	}
	if len(s.Items) == 0 {
		return errors.New("aethercore implementation backlog requires items")
	}
	...
	for i, item := range s.Items {
		...
		if _, found := seen[item.ItemID]; found {
			return fmt.Errorf("duplicate aethercore implementation backlog item %s", item.ItemID)
		}
		seen[item.ItemID] = struct{}{}
		if i > 0 {
			if backlogPriorityRank(previousPriority) > backlogPriorityRank(item.Priority) {
				return errors.New("aethercore implementation backlog priorities must be sorted canonically")
			}
			if previousPriority == item.Priority && previousItem >= item.ItemID {
				return errors.New("aethercore implementation backlog items must be sorted canonically")
			}
		}
	...
	if s.Root != "" {
		if err := ValidateHash("aethercore implementation backlog root", s.Root); err != nil {
			return err
		}
	}
	return nil
...
func (s ImplementationBacklogSpec) Validate() error {
	...
	if s.Root == "" {
		return errors.New("aethercore implementation backlog root is required")
	}
	expected := ComputeImplementationBacklogRoot(s.Items)
	if s.Root != expected {
		return fmt.Errorf("aethercore implementation backlog root mismatch: expected %s", expected)
	}
	return nil
}
...
func (i BacklogItem) ValidateFormat() error {
	...
	if !IsBacklogPriority(i.Priority) {
		return fmt.Errorf("unknown aethercore implementation backlog priority %q", i.Priority)
	}
	if !IsBacklogItemID(i.Priority, i.ItemID) {
		return fmt.Errorf("unknown aethercore implementation backlog item %q for priority %s", i.ItemID, i.Priority)
	}
	if i.Task == "" || i.Target == "" {
		return errors.New("aethercore implementation backlog item requires task and target")
	}
	if len(i.Acceptance) == 0 {
		return errors.New("aethercore implementation backlog item requires acceptance criteria")
	}
	if i.DescriptorHash != "" {
		if err := ValidateHash("aethercore implementation backlog descriptor hash", i.DescriptorHash); err != nil {
			return err
		}
	}
	return nil
...
func (i BacklogItem) Validate() error {
	...
	if i.DescriptorHash == "" {
		return errors.New("aethercore implementation backlog descriptor hash is required")
	}
	expected := ComputeBacklogItemHash(i)
	if i.DescriptorHash != expected {
		return fmt.Errorf("aethercore implementation backlog descriptor hash mismatch: expected %s", expected)
	}
	return nil
}
	...
	for priority, itemIDs := range required {
		...
		for _, itemID := range itemIDs {
			if _, found := seen[itemID]; !found {
				return fmt.Errorf("aethercore implementation backlog coverage missing %s item %s", priority, itemID)
			}
		}
	}
	return nil
...
func ComputeImplementationBacklogRoot(items []BacklogItem) string {
	items = normalizeBacklogItems(items)
	return hashRoot("aetheris-aek-implementation-backlog-v1", func(w byteWriter) {
...
func ComputeBacklogItemHash(item BacklogItem) string {
	item = item.Normalize()
	return hashRoot("aetheris-aek-implementation-backlog-item-v1", func(w byteWriter) {
		writePart(w, string(item.Priority))
		writePart(w, string(item.ItemID))
