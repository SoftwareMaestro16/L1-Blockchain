...
func (s ConflictReductionSpec) ValidateFormat() error {
	...
	if s.Version != ConflictReductionSpecVersion {
		return fmt.Errorf("aethercore conflict reduction spec version must be %d", ConflictReductionSpecVersion)
	}
	if len(s.Rules) == 0 {
		return errors.New("aethercore conflict reduction spec requires rules")
	}
	...
	for i, rule := range s.Rules {
		...
		if _, found := seen[rule.RuleID]; found {
			return fmt.Errorf("duplicate aethercore conflict reduction rule %s", rule.RuleID)
		}
		seen[rule.RuleID] = struct{}{}
		if i > 0 && previous >= rule.RuleID {
			return errors.New("aethercore conflict reduction rules must be sorted canonically")
		}
		previous = rule.RuleID
	}
	if s.Root != "" {
		if err := ValidateHash("aethercore conflict reduction spec root", s.Root); err != nil {
			return err
		}
	}
	return nil
...
func (s ConflictReductionSpec) Validate() error {
	...
	if s.Root == "" {
		return errors.New("aethercore conflict reduction spec root is required")
	}
	expected := ComputeConflictReductionSpecRoot(s.Rules)
	if s.Root != expected {
		return fmt.Errorf("aethercore conflict reduction spec root mismatch: expected %s", expected)
	}
	return nil
}
...
func (r ConflictReductionRule) ValidateFormat() error {
	...
	if !IsConflictReductionRuleID(r.RuleID) {
		return fmt.Errorf("unknown aethercore conflict reduction rule %q", r.RuleID)
	}
	if r.Rule == "" || r.Enforcement == "" || r.Evidence == "" {
		return errors.New("aethercore conflict reduction rule requires rule, enforcement, and evidence")
	}
	if r.DescriptorHash != "" {
		if err := ValidateHash("aethercore conflict reduction descriptor hash", r.DescriptorHash); err != nil {
			return err
		}
	}
	return nil
...
func (r ConflictReductionRule) Validate() error {
	...
	if r.DescriptorHash == "" {
		return errors.New("aethercore conflict reduction descriptor hash is required")
	}
	if expected := ComputeConflictReductionRuleHash(r); r.DescriptorHash != expected {
		return fmt.Errorf("aethercore conflict reduction descriptor hash mismatch: expected %s", expected)
	}
	return nil
}
...
func ValidateConflictReductionCoverage() error {
	...
	for _, ruleID := range required {
		if _, found := seen[ruleID]; !found {
			return fmt.Errorf("aethercore conflict reduction spec missing %s", ruleID)
		}
	}
	return nil
}
...
func (s StoreV2UsageSpec) ValidateFormat() error {
	...
	if s.Version != StoreV2UsageSpecVersion {
		return fmt.Errorf("aethercore Store v2 usage spec version must be %d", StoreV2UsageSpecVersion)
	}
	if len(s.Layouts) == 0 {
		return errors.New("aethercore Store v2 usage spec requires layouts")
	}
	if len(s.Benchmarks) == 0 {
		return errors.New("aethercore Store v2 usage spec requires benchmarks")
	}
	...
	for i, layout := range s.Layouts {
		...
		if _, found := layoutIDs[layout.LayoutID]; found {
			return fmt.Errorf("duplicate aethercore Store v2 layout %s", layout.LayoutID)
		}
		layoutIDs[layout.LayoutID] = struct{}{}
		if i > 0 && previousLayout >= layout.LayoutID {
			return errors.New("aethercore Store v2 layouts must be sorted canonically")
		}
	...
	for i, benchmark := range s.Benchmarks {
		...
		if _, found := layoutIDs[benchmark.LayoutID]; !found {
			return fmt.Errorf("aethercore Store v2 benchmark %s references missing layout %s", benchmark.BenchmarkID, benchmark.LayoutID)
		}
		if _, found := seenBenchmarks[benchmark.BenchmarkID]; found {
			return fmt.Errorf("duplicate aethercore Store v2 benchmark %s", benchmark.BenchmarkID)
		}
		seenBenchmarks[benchmark.BenchmarkID] = struct{}{}
		if i > 0 && previousBenchmark >= benchmark.BenchmarkID {
			return errors.New("aethercore Store v2 benchmarks must be sorted canonically")
		}
		previousBenchmark = benchmark.BenchmarkID
	}
	if s.Root != "" {
		if err := ValidateHash("aethercore Store v2 usage spec root", s.Root); err != nil {
			return err
		}
	}
	return nil
...
func (s StoreV2UsageSpec) Validate() error {
	...
	if s.Root == "" {
		return errors.New("aethercore Store v2 usage spec root is required")
	}
	expected := ComputeStoreV2UsageSpecRoot(s.Layouts, s.Benchmarks)
	if s.Root != expected {
		return fmt.Errorf("aethercore Store v2 usage spec root mismatch: expected %s", expected)
	}
	return nil
}
...
func (l StoreV2LayoutDescriptor) Validate() error {
	l = l.Normalize()
	if err := validateToken("aethercore Store v2 layout id", l.LayoutID, MaxScopeLength); err != nil {
		return err
	}
	if !IsStoreV2LayoutKind(l.Kind) {
		return fmt.Errorf("unknown aethercore Store v2 layout kind %q", l.Kind)
	}
	if len(l.StateClasses) == 0 {
		return errors.New("aethercore Store v2 layout requires state classes")
	}
	if err := validateCapabilitiesForField("aethercore Store v2 state class", l.StateClasses); err != nil {
		return err
	}
	if l.KeyPattern == "" || l.ProofSurface == "" {
		return errors.New("aethercore Store v2 layout requires key pattern and proof surface")
	}
	if err := ValidateHash("aethercore Store v2 layout descriptor hash", l.DescriptorHash); err != nil {
		return err
	}
	if expected := ComputeStoreV2LayoutDescriptorHash(l); l.DescriptorHash != expected {
		return fmt.Errorf("aethercore Store v2 layout descriptor hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (b StoreV2BenchmarkRequirement) Validate() error {
	...
	if !IsStoreV2BenchmarkID(b.BenchmarkID) {
		return fmt.Errorf("unknown aethercore Store v2 benchmark %q", b.BenchmarkID)
	}
	if b.Operation == "" || b.LayoutID == "" || b.Metric == "" {
		return errors.New("aethercore Store v2 benchmark requires operation, layout, and metric")
	}
	if err := validateToken("aethercore Store v2 benchmark layout id", b.LayoutID, MaxScopeLength); err != nil {
		return err
	}
	if b.Required && b.MaxRangeItems == 0 {
		return errors.New("aethercore Store v2 benchmark requires positive range bound")
	}
	if err := ValidateHash("aethercore Store v2 benchmark descriptor hash", b.DescriptorHash); err != nil {
		return err
	}
	if expected := ComputeStoreV2BenchmarkDescriptorHash(b); b.DescriptorHash != expected {
		return fmt.Errorf("aethercore Store v2 benchmark descriptor hash mismatch: expected %s", expected)
	}
	return nil
}
...
func ValidateStoreV2UsageCoverage() error {
	...
	for _, kind := range requiredLayouts {
		if _, found := seenLayouts[kind]; !found {
			return fmt.Errorf("aethercore Store v2 usage spec missing layout kind %s", kind)
		}
	}
	...
	for _, benchmarkID := range requiredBenchmarks {
		if _, found := seenBenchmarks[benchmarkID]; !found {
			return fmt.Errorf("aethercore Store v2 usage spec missing benchmark %s", benchmarkID)
		}
	}
	return nil
}
...
func ComputeConflictReductionRuleHash(rule ConflictReductionRule) string {
	rule = rule.Normalize()
	return hashParts("aetheris-conflict-reduction-rule-v1", string(rule.RuleID), rule.Rule, rule.Enforcement, rule.Evidence)
}

func ComputeConflictReductionSpecRoot(rules []ConflictReductionRule) string {
	ordered := normalizeConflictReductionRules(rules)
	parts := []string{"aetheris-conflict-reduction-spec-v1", fmt.Sprintf("%020d", ConflictReductionSpecVersion)}
...
func ComputeStoreV2LayoutDescriptorHash(layout StoreV2LayoutDescriptor) string {
	...
	parts := []string{"aetheris-store-v2-layout-v1", layout.LayoutID, string(layout.Kind), layout.KeyPattern, layout.ProofSurface, fmt.Sprint(layout.BoundedScan)}
...
func ComputeStoreV2BenchmarkDescriptorHash(benchmark StoreV2BenchmarkRequirement) string {
	...
	return hashParts("aetheris-store-v2-benchmark-v1", string(benchmark.BenchmarkID), benchmark.Operation, benchmark.LayoutID, benchmark.Metric, fmt.Sprint(benchmark.MaxRangeItems), fmt.Sprint(benchmark.Required))
}
...
func ComputeStoreV2UsageSpecRoot(layouts []StoreV2LayoutDescriptor, benchmarks []StoreV2BenchmarkRequirement) string {
	...
	parts := []string{"aetheris-store-v2-usage-spec-v1", fmt.Sprintf("%020d", StoreV2UsageSpecVersion)}
