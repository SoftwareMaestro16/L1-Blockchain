...
func (m GlobalStatePrefixModel) ValidateFormat() error {
	...
	if m.Version != StatePrefixModelVersion {
		return fmt.Errorf("aethercore state prefix model version must be %d", StatePrefixModelVersion)
	}
	if len(m.Entries) == 0 {
		return errors.New("aethercore state prefix model requires entries")
	}
	...
	for _, entry := range m.Entries {
		...
		if _, found := seenPrefixes[entry.Prefix]; found {
			return fmt.Errorf("aethercore duplicate state prefix %s", entry.Prefix)
		}
		seenPrefixes[entry.Prefix] = struct{}{}
		key := string(entry.Namespace) + "/" + entry.Prefix
		if previousKey != "" && previousKey >= key {
			return errors.New("aethercore state prefixes must be sorted canonically")
		}
		previousKey = key
	}
	if m.Root != "" {
		if err := ValidateHash("aethercore state prefix model root", m.Root); err != nil {
			return err
		}
	}
	return nil
...
func (m GlobalStatePrefixModel) Validate() error {
	...
	if m.Root == "" {
		return errors.New("aethercore state prefix model root is required")
	}
	expected := ComputeGlobalStatePrefixModelRoot(m.Entries)
	if m.Root != expected {
		return fmt.Errorf("aethercore state prefix model root mismatch: expected %s", expected)
	}
	return nil
}
...
func BuildGlobalStatePrefixDescriptor(entry GlobalStatePrefixDescriptor) (GlobalStatePrefixDescriptor, error) {
	...
	if entry.DescriptorHash != "" {
		return GlobalStatePrefixDescriptor{}, errors.New("aethercore state prefix descriptor hash must be empty before construction")
	}
...
func (entry GlobalStatePrefixDescriptor) ValidateFormat() error {
	...
	if !IsStatePrefixNamespace(entry.Namespace) {
		return fmt.Errorf("unknown aethercore state prefix namespace %q", entry.Namespace)
	}
	if err := validateStatePrefixPattern(entry.Namespace, entry.Prefix); err != nil {
		return err
	}
	if entry.Purpose == "" {
		return errors.New("aethercore state prefix purpose is required")
	}
	if entry.ProofScope == "" {
		return errors.New("aethercore state prefix proof scope is required")
	}
	if entry.DescriptorHash != "" {
		if err := ValidateHash("aethercore state prefix descriptor hash", entry.DescriptorHash); err != nil {
			return err
		}
	}
	return nil
...
func (entry GlobalStatePrefixDescriptor) Validate() error {
	...
	if entry.DescriptorHash == "" {
		return errors.New("aethercore state prefix descriptor hash is required")
	}
	expected := ComputeGlobalStatePrefixDescriptorHash(entry)
	if entry.DescriptorHash != expected {
		return fmt.Errorf("aethercore state prefix descriptor hash mismatch: expected %s", expected)
	}
	return nil
}
...
func ComputeGlobalStatePrefixDescriptorHash(entry GlobalStatePrefixDescriptor) string {
	entry = entry.Normalize()
	return hashParts(
		"aetheris-global-state-prefix-descriptor-v1",
...
func ComputeGlobalStatePrefixModelRoot(entries []GlobalStatePrefixDescriptor) string {
	...
	parts := []string{"aetheris-global-state-prefix-model-v1", fmt.Sprintf("%020d", StatePrefixModelVersion)}
...
func ValidateGlobalStatePrefixModel() error {
	...
	for _, key := range required {
		if _, found := seen[key]; !found {
			return fmt.Errorf("aethercore state prefix model missing %s", key)
		}
	}
	return nil
}
...
func validateStatePrefixPattern(namespace StatePrefixNamespace, prefix string) error {
	if strings.TrimSpace(prefix) != prefix || prefix == "" {
		return errors.New("aethercore state prefix is required and must not have surrounding whitespace")
	}
	if strings.Contains(prefix, "//") {
		return errors.New("aethercore state prefix must not contain empty segments")
	}
	for _, r := range prefix {
		if r <= ' ' || r == 0x7f {
			return errors.New("aethercore state prefix must not contain whitespace or control characters")
		}
		if r >= 'A' && r <= 'Z' || r >= 'a' && r <= 'z' || r >= '0' && r <= '9' ||
			r == '_' || r == '-' || r == '/' || r == '*' || r == '{' || r == '}' {
			continue
		}
		return fmt.Errorf("aethercore state prefix contains invalid character %q", r)
	}
	switch namespace {
	case StatePrefixNamespaceCore:
		if !strings.HasPrefix(prefix, "core/") {
			return errors.New("aethercore core state prefix must start with core/")
		}
		if strings.Contains(prefix, "{zone_id}") || strings.Contains(prefix, "{shard_id}") {
			return errors.New("aethercore core state prefix cannot contain zone or shard placeholders")
		}
	case StatePrefixNamespaceZone:
		if !strings.HasPrefix(prefix, "zone/{zone_id}/") {
			return errors.New("aethercore zone state prefix must start with zone/{zone_id}/")
		}
		if strings.Contains(prefix, "{shard_id}") {
			return errors.New("aethercore zone state prefix cannot contain shard placeholder")
		}
	case StatePrefixNamespaceShard:
		if !strings.HasPrefix(prefix, "zone/{zone_id}/shard/{shard_id}/") {
			return errors.New("aethercore shard state prefix must start with zone/{zone_id}/shard/{shard_id}/")
		}
	default:
		return fmt.Errorf("unknown aethercore state prefix namespace %q", namespace)
	}
	if strings.Count(prefix, "*") > 1 {
		return errors.New("aethercore state prefix supports at most one wildcard")
	}
	if strings.Contains(prefix, "*") && !strings.HasSuffix(prefix, "*") {
		return errors.New("aethercore state prefix wildcard must be terminal")
	}
	if strings.Contains(prefix, "{zone_id}") && strings.Count(prefix, "{zone_id}") != 1 {
		return errors.New("aethercore state prefix must contain at most one zone placeholder")
	}
	if strings.Contains(prefix, "{shard_id}") && strings.Count(prefix, "{shard_id}") != 1 {
		return errors.New("aethercore state prefix must contain at most one shard placeholder")
	}
	return nil
}
