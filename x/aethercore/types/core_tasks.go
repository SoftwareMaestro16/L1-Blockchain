...
func DefaultCoreImplementationTasks() ([]CoreImplementationTaskSpec, error) {
	tasks := []CoreImplementationTaskSpec{
		coreTask(CoreTaskPriorityP0, CoreTaskZoneRegistry, "Zone registry", "x/aethercore keeper and types", []string{"register-zone-descriptors", "update-zone-descriptors", "disable-zone-descriptors", "export-zone-descriptors", "validate-canonical-ordering", "enforce-version-gates"}),
		...
		coreTask(CoreTaskPriorityP1, CoreTaskRootConsistencyInvariants, "Root consistency invariants", "x/aethercore invariants", []string{"assert-zone-roots", "assert-message-roots", "assert-receipt-roots", "assert-routing-table-roots", "assert-shard-layout-roots", "assert-global-root"}),
		coreTask(CoreTaskPriorityP1, CoreTaskBlockReplayTests, "Block replay tests", "x/aethercore tests", []string{"rebuild-state-from-different-input-orders", "require-identical-zone-commitments", "require-identical-proof-roots", "require-identical-global-app-hash"}),
		coreTask(CoreTaskPriorityP1, CoreTaskKeeperIntegration, "Keeper integration", "x/aethercore/keeper", []string{"persist-descriptors", "persist-layouts", "persist-routing-tables", "persist-commitments", "persist-root-snapshots", "persist-finality-records"}),
...
func (t CoreImplementationTaskSpec) ValidateHash() error {
	...
	if t.TaskHash != expected {
		return fmt.Errorf("aethercore implementation task hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (t CoreImplementationTaskSpec) ValidateFormat() error {
	if !isCoreImplementationPriority(t.Priority) {
		return fmt.Errorf("unknown aethercore implementation task priority %q", t.Priority)
	}
	if err := validatePolicyID("aethercore implementation task id", string(t.TaskID)); err != nil {
		return err
	}
	if err := validateTopologyLabel("aethercore implementation task name", t.Task); err != nil {
		return err
	}
	if err := validateTopologyLabel("aethercore implementation task target", t.Target); err != nil {
		return err
	}
	if len(t.AcceptanceCriteria) == 0 {
		return errors.New("aethercore implementation task requires acceptance criteria")
	}
	if err := validateCapabilitiesForField("aethercore implementation acceptance criterion", t.AcceptanceCriteria); err != nil {
		return err
	}
	if t.TaskHash != "" {
		return ValidateHash("aethercore implementation task hash", t.TaskHash)
	}
	return nil
}
...
func ComputeCoreImplementationTaskHash(task CoreImplementationTaskSpec) string {
	...
	parts := []string{"aetheris-aek-core-implementation-task-v1", string(task.Priority), string(task.TaskID), task.Task, task.Target}
...
func ComputeCoreImplementationReadinessHash(result CoreImplementationReadiness) string {
	...
	sortCoreTaskIDs(missing)
	sortCoreTaskIDs(p0)
	parts := []string{"aetheris-aek-core-implementation-readiness-v1", fmt.Sprint(result.TaskCount), fmt.Sprint(result.Ready)}
...
func validateCoreImplementationTasks(tasks []CoreImplementationTaskSpec) error {
	if len(tasks) == 0 {
		return errors.New("aethercore implementation tasks are required")
	}
	...
	for _, task := range tasks {
		...
		if _, found := seen[task.TaskID]; found {
			return fmt.Errorf("duplicate aethercore implementation task %s", task.TaskID)
		}
	...
	for _, taskID := range []CoreImplementationTaskID{
		...
		if _, found := seen[taskID]; !found {
			return fmt.Errorf("aethercore implementation task %s is missing", taskID)
		}
	}
	return nil
}
