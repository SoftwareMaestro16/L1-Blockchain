...
func (m StateInvariantModel) ValidateFormat() error {
	...
	if m.Version != StateInvariantModelVersion {
		return fmt.Errorf("aethercore state invariant model version must be %d", StateInvariantModelVersion)
	}
	if len(m.Invariants) == 0 {
		return errors.New("aethercore state invariant model requires invariants")
	}
	...
	for i, invariant := range m.Invariants {
		...
		if _, found := seen[invariant.InvariantID]; found {
			return fmt.Errorf("aethercore duplicate state invariant %s", invariant.InvariantID)
		}
		seen[invariant.InvariantID] = struct{}{}
		if i > 0 && previous >= invariant.InvariantID {
			return errors.New("aethercore state invariants must be sorted canonically")
		}
		previous = invariant.InvariantID
	}
	if m.Root != "" {
		if err := ValidateHash("aethercore state invariant model root", m.Root); err != nil {
			return err
		}
	}
	return nil
...
func (m StateInvariantModel) Validate() error {
	...
	if m.Root == "" {
		return errors.New("aethercore state invariant model root is required")
	}
	expected := ComputeStateInvariantModelRoot(m.Invariants)
	if m.Root != expected {
		return fmt.Errorf("aethercore state invariant model root mismatch: expected %s", expected)
	}
	return nil
}
...
func BuildStateInvariantDescriptor(invariant StateInvariantDescriptor) (StateInvariantDescriptor, error) {
	...
	if invariant.DescriptorHash != "" {
		return StateInvariantDescriptor{}, errors.New("aethercore state invariant descriptor hash must be empty before construction")
	}
...
func (invariant StateInvariantDescriptor) ValidateFormat() error {
	...
	if !IsStateInvariantID(invariant.InvariantID) {
		return fmt.Errorf("unknown aethercore state invariant %q", invariant.InvariantID)
	}
	if invariant.Invariant == "" {
		return errors.New("aethercore state invariant text is required")
	}
	if invariant.Enforcement == "" {
		return errors.New("aethercore state invariant enforcement is required")
	}
	if invariant.DescriptorHash != "" {
		if err := ValidateHash("aethercore state invariant descriptor hash", invariant.DescriptorHash); err != nil {
			return err
		}
	}
	return nil
...
func (invariant StateInvariantDescriptor) Validate() error {
	...
	if invariant.DescriptorHash == "" {
		return errors.New("aethercore state invariant descriptor hash is required")
	}
	expected := ComputeStateInvariantDescriptorHash(invariant)
	if invariant.DescriptorHash != expected {
		return fmt.Errorf("aethercore state invariant descriptor hash mismatch: expected %s", expected)
	}
	return nil
}
...
func BuildStateInvariantEvidence(evidence StateInvariantEvidence) (StateInvariantEvidence, error) {
	...
	if evidence.EvidenceHash != "" {
		return StateInvariantEvidence{}, errors.New("aethercore state invariant evidence hash must be empty before construction")
	}
...
func (evidence StateInvariantEvidence) ValidateFormat() error {
	...
	if evidence.Height == 0 {
		return errors.New("aethercore state invariant evidence height must be positive")
	}
	if !evidence.ZoneCommitmentMatchesExecutedState {
		return errors.New("aethercore state invariant failed: zone commitment must match executed state")
	}
	if !evidence.ShardRootsIncludedInZoneRoot {
		return errors.New("aethercore state invariant failed: shard roots must be included in zone root")
	}
	if !evidence.OutputMessagesIncludedInSourceOutbox {
		return errors.New("aethercore state invariant failed: output messages must be included in source outbox")
	}
	if !evidence.ConsumedMessagesHaveOneReceipt {
		return errors.New("aethercore state invariant failed: consumed messages must have exactly one receipt")
	}
	if !evidence.CrossZoneValueTransferConservesNaet {
		return errors.New("aethercore state invariant failed: cross-zone value transfer must conserve naet")
	}
	if !evidence.PaymentCollateralMatchesLockedBalance {
		return errors.New("aethercore state invariant failed: payment collateral must match locked bank balance")
	}
	if !evidence.IdentityDomainsHaveOwnershipBinding {
		return errors.New("aethercore state invariant failed: identity domain ownership binding is required")
	}
	if !evidence.ContractStorageProofsBindToZoneRoot {
		return errors.New("aethercore state invariant failed: contract storage proof must bind to contract zone root")
	}
	for _, item := range []struct {
		name  string
		value string
	}{
		{"aethercore state invariant zone commitment root", evidence.ZoneCommitmentRoot},
		{"aethercore state invariant shard roots root", evidence.ShardRootsRoot},
		{"aethercore state invariant message outbox root", evidence.MessageOutboxRoot},
		{"aethercore state invariant message receipt root", evidence.MessageReceiptRoot},
		{"aethercore state invariant value conservation root", evidence.ValueConservationRoot},
		{"aethercore state invariant payment collateral root", evidence.PaymentCollateralRoot},
		{"aethercore state invariant identity ownership root", evidence.IdentityOwnershipRoot},
		{"aethercore state invariant contract storage proof root", evidence.ContractStorageProofRoot},
	} {
	...
	if evidence.EvidenceHash != "" {
		if err := ValidateHash("aethercore state invariant evidence hash", evidence.EvidenceHash); err != nil {
			return err
		}
	}
	return nil
...
func (evidence StateInvariantEvidence) Validate() error {
	...
	if evidence.EvidenceHash == "" {
		return errors.New("aethercore state invariant evidence hash is required")
	}
	expected := ComputeStateInvariantEvidenceHash(evidence)
	if evidence.EvidenceHash != expected {
		return fmt.Errorf("aethercore state invariant evidence hash mismatch: expected %s", expected)
	}
	return nil
}
...
func ComputeStateInvariantDescriptorHash(invariant StateInvariantDescriptor) string {
	invariant = invariant.Normalize()
	return hashParts(
		"aetheris-state-invariant-descriptor-v1",
...
func ComputeStateInvariantModelRoot(invariants []StateInvariantDescriptor) string {
	...
	parts := []string{"aetheris-state-invariant-model-v1", fmt.Sprintf("%020d", StateInvariantModelVersion)}
...
func ComputeStateInvariantEvidenceHash(evidence StateInvariantEvidence) string {
	evidence = evidence.Normalize()
	return hashParts(
		"aetheris-state-invariant-evidence-v1",
...
func ValidateStateInvariantModel() error {
	...
	for _, invariantID := range required {
		if _, found := seen[invariantID]; !found {
			return fmt.Errorf("aethercore state invariant model missing %s", invariantID)
		}
	}
	return nil
}
