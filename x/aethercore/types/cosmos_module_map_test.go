...
func TestCosmosSDKNewModuleMapCoversSectionTenOne(t *testing.T) {
	...
	require.Equal(t, "Core", byModule["x/aethercore"].Zone)
	require.Contains(t, byModule["x/aethercore"].Responsibility, "Zone registry")
	require.Contains(t, byModule["x/aethercore"].AcceptanceSignal, "identical zone commitments")

	require.Equal(t, "Core + zones", byModule["x/msgbus"].Zone)
	require.True(t, slices.Contains(byModule["x/msgbus"].Dependencies, "x/aethercore"))
