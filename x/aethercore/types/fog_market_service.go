...
func NewFogProviderRegistry(descriptor ServiceDescriptor) (FogProviderRegistry, error) {
	...
	if descriptor.ServiceType != ServiceTypeFogMarket {
		return FogProviderRegistry{}, errors.New("aethercore fog registry requires fog market service descriptor")
	}
...
func RegisterFogProvider(registry FogProviderRegistry, provider FogProviderRecord) (FogProviderRegistry, FogProviderRecord, error) {
	...
	if _, found := registry.ProviderByID(provider.ProviderID); found {
		return FogProviderRegistry{}, FogProviderRecord{}, fmt.Errorf("aethercore fog provider %s already registered", provider.ProviderID)
	}
	next := registry.clone()
...
func RenewFogProvider(registry FogProviderRegistry, providerID string, commitment FogAvailabilityCommitment, expiryHeight uint64, updatedHeight uint64) (FogProviderRegistry, FogProviderRecord, error) {
	...
	if !found {
		return FogProviderRegistry{}, FogProviderRecord{}, fmt.Errorf("aethercore fog provider %s not found", providerID)
	}
	if updatedHeight == 0 || updatedHeight < provider.UpdatedHeight {
		return FogProviderRegistry{}, FogProviderRecord{}, errors.New("aethercore fog provider renewal height is invalid")
	}
	if expiryHeight <= provider.ExpiryHeight {
		return FogProviderRegistry{}, FogProviderRecord{}, errors.New("aethercore fog provider renewal must extend expiry")
	}
...
func UpdateFogProviderReputation(registry FogProviderRegistry, event FogReputationEvent) (FogProviderRegistry, FogProviderRecord, error) {
	...
	if !found {
		return FogProviderRegistry{}, FogProviderRecord{}, fmt.Errorf("aethercore fog provider %s not found", event.ProviderID)
	}
...
func SelectFogProviders(registry FogProviderRegistry, policy FogProviderSelectionPolicy, currentHeight uint64) (FogProviderSelection, error) {
	...
	if currentHeight == 0 {
		return FogProviderSelection{}, errors.New("aethercore fog provider selection height must be positive")
	}
...
func OpenFogProviderDispute(registry FogProviderRegistry, dispute FogProviderDispute) (FogProviderRegistry, FogProviderDispute, error) {
	...
	if _, found := registry.ProviderByID(dispute.ProviderID); !found {
		return FogProviderRegistry{}, FogProviderDispute{}, fmt.Errorf("aethercore fog provider %s not found", dispute.ProviderID)
	}
	...
	if _, found := registry.DisputeByID(dispute.DisputeID); found {
		return FogProviderRegistry{}, FogProviderDispute{}, fmt.Errorf("aethercore fog dispute %s already exists", dispute.DisputeID)
	}
	next := registry.clone()
...
func SlashFogProvider(registry FogProviderRegistry, disputeID string, proofAccepted bool, slashedHeight uint64) (FogProviderRegistry, FogProviderSlash, error) {
	...
	if !found {
		return FogProviderRegistry{}, FogProviderSlash{}, fmt.Errorf("aethercore fog dispute %s not found", disputeID)
	}
	if dispute.Status != FogDisputeOpen {
		return FogProviderRegistry{}, FogProviderSlash{}, errors.New("aethercore fog dispute is not open")
	}
	if slashedHeight == 0 || slashedHeight > dispute.ResolveByHeight {
		return FogProviderRegistry{}, FogProviderSlash{}, errors.New("aethercore fog slashing is outside dispute window")
	}
	provider, providerIndex, found := registry.providerByIDWithIndex(dispute.ProviderID)
	if !found {
		return FogProviderRegistry{}, FogProviderSlash{}, errors.New("aethercore fog dispute provider is not registered")
	}
	next := registry.clone()
	if !proofAccepted {
	...
	if err := mixedAmountAtLeast("aethercore fog provider collateral", provider.CollateralAmount, slash.PenaltyAmount); err != nil {
		return FogProviderRegistry{}, FogProviderSlash{}, err
	}
...
func (provider FogProviderRecord) ValidateForRegistry(registry FogProviderRegistry) error {
	...
	if provider.CollateralDenom != registry.CollateralDenom {
		return errors.New("aethercore fog provider collateral denom mismatch")
	}
	return mixedAmountAtLeast("aethercore fog provider collateral", provider.CollateralAmount, registry.MinCollateralAmount)
}

func (provider FogProviderRecord) Validate() error {
	if err := validatePolicyID("aethercore fog provider id", provider.ProviderID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore fog provider identity key", provider.IdentityKey); err != nil {
		return err
	}
	if !IsFogServiceCategory(provider.Category) {
		return fmt.Errorf("unknown aethercore fog service category %q", provider.Category)
	}
	if err := provider.Pricing.Validate(); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore fog provider collateral denom", provider.CollateralDenom); err != nil {
		return err
	}
	if err := validateAmountString("aethercore fog provider collateral amount", provider.CollateralAmount); err != nil {
		return err
	}
	if err := validateAmountString("aethercore fog provider stake amount", provider.StakeAmount); err != nil {
		return err
	}
	if err := provider.AvailabilityCommitment.Validate(); err != nil {
		return err
	}
	if err := validateSortedStringSet("aethercore fog provider supported interface", provider.SupportedInterfaces); err != nil {
		return err
	}
	if len(provider.SupportedInterfaces) == 0 {
		return errors.New("aethercore fog provider requires supported interfaces")
	}
	if !IsFogProviderStatus(provider.Status) {
		return fmt.Errorf("unknown aethercore fog provider status %q", provider.Status)
	}
	if provider.RegisteredHeight == 0 || provider.UpdatedHeight < provider.RegisteredHeight {
		return errors.New("aethercore fog provider heights are invalid")
	}
	if provider.ExpiryHeight <= provider.UpdatedHeight {
		return errors.New("aethercore fog provider expiry must exceed updated height")
	}
	if err := ValidateHash("aethercore fog provider hash", provider.ProviderHash); err != nil {
		return err
	}
	if expected := ComputeFogProviderHash(provider); provider.ProviderHash != expected {
		return fmt.Errorf("aethercore fog provider hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (pricing FogProviderPricing) Validate() error {
	if err := validatePolicyID("aethercore fog provider pricing denom", pricing.Denom); err != nil {
		return err
	}
	if err := validateAmountString("aethercore fog provider pricing amount", pricing.Amount); err != nil {
		return err
	}
	if pricing.MaxAmount != "" {
		if err := validateAmountString("aethercore fog provider pricing max amount", pricing.MaxAmount); err != nil {
			return err
		}
		if err := mixedAmountAtLeast("aethercore fog provider pricing max amount", pricing.MaxAmount, pricing.Amount); err != nil {
			return err
		}
	}
	if !IsFogPricingUnit(pricing.Unit) {
		return fmt.Errorf("unknown aethercore fog pricing unit %q", pricing.Unit)
	}
	return validateOptionalHash("aethercore fog provider pricing model hash", pricing.ModelHash)
}
...
func (commitment FogAvailabilityCommitment) Validate() error {
	if err := ValidateHash("aethercore fog availability commitment hash", commitment.CommitmentHash); err != nil {
		return err
	}
	if err := ValidateHash("aethercore fog availability endpoint hash", commitment.EndpointHash); err != nil {
		return err
	}
	if commitment.WindowStart == 0 || commitment.WindowEnd <= commitment.WindowStart {
		return errors.New("aethercore fog availability window is invalid")
	}
	if commitment.UptimeTargetBps == 0 || commitment.UptimeTargetBps > 10_000 {
		return errors.New("aethercore fog availability target must be 1..10000 bps")
	}
	if commitment.RenewalNonce == 0 {
		return errors.New("aethercore fog availability renewal nonce must be positive")
	}
	if err := ValidateHash("aethercore fog availability signature hash", commitment.SignatureHash); err != nil {
		return err
	}
	if expected := ComputeFogAvailabilityCommitmentHash(commitment); commitment.CommitmentHash != expected {
		return fmt.Errorf("aethercore fog availability commitment hash mismatch: expected %s", expected)
	}
	return nil
}

func (registry FogProviderRegistry) Validate() error {
	if err := validatePolicyID("aethercore fog registry service id", registry.ServiceID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore fog registry provider pool id", registry.ProviderPoolID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore fog registry collateral denom", registry.CollateralDenom); err != nil {
		return err
	}
	if err := validateAmountString("aethercore fog registry min collateral", registry.MinCollateralAmount); err != nil {
		return err
	}
	...
	if err := ValidateHash("aethercore fog registry hash", registry.RegistryHash); err != nil {
		return err
	}
	if expected := ComputeFogProviderRegistryHash(registry); registry.RegistryHash != expected {
		return fmt.Errorf("aethercore fog registry hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (provider FogProviderRecord) Selectable(policy FogProviderSelectionPolicy, currentHeight uint64) bool {
	...
	if policy.MaxPriceAmount != "" {
		if err := mixedAmountAtMost("aethercore fog provider price", provider.Pricing.Amount, policy.MaxPriceAmount); err != nil {
			return false
		}
	}
	return true
...
func (policy FogProviderSelectionPolicy) Validate() error {
	if !IsFogServiceCategory(policy.Category) {
		return fmt.Errorf("unknown aethercore fog selection category %q", policy.Category)
	}
	if policy.RequiredInterface != "" {
		if err := validatePolicyID("aethercore fog selection interface", policy.RequiredInterface); err != nil {
			return err
		}
	}
	if !IsFogSelectionStrategy(policy.Strategy) {
		return fmt.Errorf("unknown aethercore fog selection strategy %q", policy.Strategy)
	}
	if policy.MaxPriceAmount != "" {
		if err := validateAmountString("aethercore fog selection max price", policy.MaxPriceAmount); err != nil {
			return err
		}
	}
	if policy.Limit == 0 {
		return errors.New("aethercore fog selection limit must be positive")
	}
	return validatePolicyID("aethercore fog selection nonce", policy.SelectionNonce)
}
...
func (selection FogProviderSelection) Validate() error {
	...
	if len(selection.ProviderIDs) == 0 {
		return errors.New("aethercore fog selection requires providers")
	}
	if err := validateFogSelectionProviders(selection.ProviderIDs); err != nil {
		return err
	}
	if err := ValidateHash("aethercore fog selection hash", selection.SelectionHash); err != nil {
		return err
	}
	if expected := ComputeFogProviderSelectionHash(selection); selection.SelectionHash != expected {
		return fmt.Errorf("aethercore fog selection hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (event FogReputationEvent) Validate() error {
	if err := ValidateHash("aethercore fog reputation event id", event.EventID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore fog reputation provider id", event.ProviderID); err != nil {
		return err
	}
	if event.Height == 0 {
		return errors.New("aethercore fog reputation height must be positive")
	}
	if event.Successes == 0 && event.Failures == 0 && event.ScoreDelta == 0 {
		return errors.New("aethercore fog reputation event must change score or counters")
	}
	if err := validatePolicyID("aethercore fog reputation reason", event.Reason); err != nil {
		return err
	}
	if err := ValidateHash("aethercore fog reputation event hash", event.EventHash); err != nil {
		return err
	}
	if expected := ComputeFogReputationEventHash(event); event.EventHash != expected {
		return fmt.Errorf("aethercore fog reputation event hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (dispute FogProviderDispute) ValidateForRegistry(registry FogProviderRegistry) error {
	...
	if dispute.ServiceID != registry.ServiceID {
		return errors.New("aethercore fog dispute service mismatch")
	}
	return nil
}

func (dispute FogProviderDispute) Validate() error {
	if err := ValidateHash("aethercore fog dispute id", dispute.DisputeID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore fog dispute provider id", dispute.ProviderID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore fog dispute service id", dispute.ServiceID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore fog dispute challenger", dispute.Challenger); err != nil {
		return err
	}
	if !IsMixedFaultClass(dispute.FaultClass) {
		return fmt.Errorf("unknown aethercore fog dispute fault class %q", dispute.FaultClass)
	}
	if err := ValidateHash("aethercore fog dispute evidence hash", dispute.EvidenceHash); err != nil {
		return err
	}
	if dispute.OpenedHeight == 0 || dispute.ResolveByHeight <= dispute.OpenedHeight {
		return errors.New("aethercore fog dispute window is invalid")
	}
	if !IsFogDisputeStatus(dispute.Status) {
		return fmt.Errorf("unknown aethercore fog dispute status %q", dispute.Status)
	}
	if err := ValidateHash("aethercore fog dispute hash", dispute.DisputeHash); err != nil {
		return err
	}
	if expected := ComputeFogProviderDisputeHash(dispute); dispute.DisputeHash != expected {
		return fmt.Errorf("aethercore fog dispute hash mismatch: expected %s", expected)
	}
	return nil
}
...
func (slash FogProviderSlash) ValidateForRegistry(registry FogProviderRegistry) error {
	...
	if slash.ServiceID != registry.ServiceID {
		return errors.New("aethercore fog slash service mismatch")
	}
	if slash.PenaltyDenom != registry.CollateralDenom {
		return errors.New("aethercore fog slash collateral denom mismatch")
	}
	return nil
}

func (slash FogProviderSlash) Validate() error {
	if err := ValidateHash("aethercore fog slash id", slash.SlashID); err != nil {
		return err
	}
	if err := ValidateHash("aethercore fog slash dispute id", slash.DisputeID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore fog slash provider id", slash.ProviderID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore fog slash service id", slash.ServiceID); err != nil {
		return err
	}
	if !IsMixedFaultClass(slash.FaultClass) {
		return fmt.Errorf("unknown aethercore fog slash fault class %q", slash.FaultClass)
	}
	if err := validatePolicyID("aethercore fog slash penalty denom", slash.PenaltyDenom); err != nil {
		return err
	}
	if err := validateAmountString("aethercore fog slash penalty amount", slash.PenaltyAmount); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore fog slash recipient", slash.Recipient); err != nil {
		return err
	}
	if slash.SlashedHeight == 0 {
		return errors.New("aethercore fog slash height must be positive")
	}
	if slash.ReputationDelta >= 0 {
		return errors.New("aethercore fog slash reputation delta must be negative")
	}
	if err := ValidateHash("aethercore fog slash hash", slash.SlashHash); err != nil {
		return err
	}
	if expected := ComputeFogProviderSlashHash(slash); slash.SlashHash != expected {
		return fmt.Errorf("aethercore fog slash hash mismatch: expected %s", expected)
	}
	return nil
}
...
func ComputeFogProviderRegistryHash(registry FogProviderRegistry) string {
	parts := []string{
		"aetheris-aek-fog-provider-registry-v1",
		registry.ServiceID,
...
func ComputeFogProviderPricingHash(pricing FogProviderPricing) string {
	return hashParts(
		"aetheris-aek-fog-provider-pricing-v1",
		pricing.Denom,
		pricing.Amount,
		pricing.MaxAmount,
		string(pricing.Unit),
...
func ComputeFogAvailabilityCommitmentHash(commitment FogAvailabilityCommitment) string {
	return hashParts(
		"aetheris-aek-fog-availability-v1",
...
func ComputeFogReputationEventID(event FogReputationEvent) string {
	return hashParts("aetheris-aek-fog-reputation-id-v1", event.ProviderID, fmt.Sprint(event.Height), event.Reason)
}

func ComputeFogReputationEventHash(event FogReputationEvent) string {
	return hashParts(
		"aetheris-aek-fog-reputation-v1",
		event.EventID,
		event.ProviderID,
...
func ComputeFogProviderDisputeID(dispute FogProviderDispute) string {
	return hashParts("aetheris-aek-fog-dispute-id-v1", dispute.ProviderID, dispute.Challenger, dispute.EvidenceHash)
}

func ComputeFogProviderDisputeHash(dispute FogProviderDispute) string {
	return hashParts(
		"aetheris-aek-fog-dispute-v1",
		dispute.DisputeID,
		dispute.ProviderID,
		dispute.ServiceID,
		dispute.Challenger,
...
func ComputeFogProviderSlashID(slash FogProviderSlash) string {
	return hashParts("aetheris-aek-fog-slash-id-v1", slash.DisputeID, slash.ProviderID, string(slash.FaultClass))
}

func ComputeFogProviderSlashHash(slash FogProviderSlash) string {
	return hashParts(
		"aetheris-aek-fog-slash-v1",
		slash.SlashID,
		slash.DisputeID,
		slash.ProviderID,
		slash.ServiceID,
...
func validateFogProviders(registry FogProviderRegistry) error {
	...
	for _, provider := range registry.Providers {
		...
		if _, found := seen[provider.ProviderID]; found {
			return fmt.Errorf("duplicate aethercore fog provider %s", provider.ProviderID)
		}
		seen[provider.ProviderID] = struct{}{}
		if previous != "" && previous >= provider.ProviderID {
			return errors.New("aethercore fog providers must be sorted canonically")
		}
...
func validateFogReputationEvents(events []FogReputationEvent) error {
	...
	for _, event := range events {
		...
		if _, found := seen[event.EventID]; found {
			return fmt.Errorf("duplicate aethercore fog reputation event %s", event.EventID)
		}
		seen[event.EventID] = struct{}{}
		if previous != "" && previous >= event.EventID {
			return errors.New("aethercore fog reputation events must be sorted canonically")
		}
...
func validateFogDisputes(registry FogProviderRegistry) error {
	...
	for _, dispute := range registry.Disputes {
		...
		if _, found := seen[dispute.DisputeID]; found {
			return fmt.Errorf("duplicate aethercore fog dispute %s", dispute.DisputeID)
		}
		seen[dispute.DisputeID] = struct{}{}
		if previous != "" && previous >= dispute.DisputeID {
			return errors.New("aethercore fog disputes must be sorted canonically")
		}
...
func validateFogSlashes(registry FogProviderRegistry) error {
	...
	for _, slash := range registry.Slashes {
		...
		if _, found := seen[slash.SlashID]; found {
			return fmt.Errorf("duplicate aethercore fog slash %s", slash.SlashID)
		}
		seen[slash.SlashID] = struct{}{}
		if previous != "" && previous >= slash.SlashID {
			return errors.New("aethercore fog slashes must be sorted canonically")
		}
...
func validateFogSelectionProviders(providerIDs []string) error {
	...
	for _, providerID := range providerIDs {
		if err := validatePolicyID("aethercore fog selection provider", providerID); err != nil {
			return err
		}
		if _, found := seen[providerID]; found {
			return fmt.Errorf("duplicate aethercore fog selection provider %s", providerID)
		}
