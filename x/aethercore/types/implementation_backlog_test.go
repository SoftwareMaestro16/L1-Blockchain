...
func TestImplementationBacklogRejectsWrongPriorityAndMissingAcceptance(t *testing.T) {
	wrongPriority := backlogItem(BacklogPriorityMedium, BacklogItemAetherCoreSkeleton, "Implement x/aethercore skeleton.", "x/aethercore module", []string{"keeper shell"})
	if err := wrongPriority.Validate(); err == nil {
		t.Fatal("expected backlog item with wrong priority to fail")
	}

	missingAcceptance := backlogItem(BacklogPriorityHigh, BacklogItemAetherCoreSkeleton, "Implement x/aethercore skeleton.", "x/aethercore module", []string{"keeper shell"})
	...
	badHash := backlogItem(BacklogPriorityHigh, BacklogItemAetherCoreSkeleton, "Implement x/aethercore skeleton.", "x/aethercore module", []string{"keeper shell"})
...
func TestImplementationBacklogAcceptanceCanonicalization(t *testing.T) {
	item := BacklogItem{
		...
		ItemID:   BacklogItemAetherCoreSkeleton,
		Task:     " Implement x/aethercore skeleton. ",
		Target:   " x/aethercore module ",
		Acceptance: []string{
			" keeper shell ",
			"params",
			"keeper shell",
