...
func (manifest HardConstraintManifest) ValidateFormat() error {
	...
	if len(manifest.Constraints) != len(required) {
		return fmt.Errorf("aethercore hard constraint manifest must include %d required constraints", len(required))
	}
	...
	for i, constraint := range manifest.Constraints {
		...
		if _, found := seen[constraint.ID]; found {
			return fmt.Errorf("duplicate aethercore hard constraint %s", constraint.ID)
		}
		seen[constraint.ID] = struct{}{}
		if i > 0 && previous >= constraint.ID {
			return errors.New("aethercore hard constraints must be sorted canonically")
		}
		previous = constraint.ID
	}
	for _, id := range required {
		if _, found := seen[HardConstraintID(id)]; !found {
			return fmt.Errorf("missing aethercore hard constraint %s", id)
		}
	}
	if manifest.ManifestHash != "" {
		return ValidateHash("aethercore hard constraint manifest hash", manifest.ManifestHash)
	}
	return nil
}
...
func (manifest HardConstraintManifest) Validate() error {
	...
	if manifest.ManifestHash != ComputeHardConstraintManifestHash(manifest) {
		return errors.New("aethercore hard constraint manifest hash mismatch")
	}
	return nil
}
...
func (constraint HardConstraint) ValidateFormat() error {
	...
	if !IsRequiredHardConstraintID(constraint.ID) {
		return fmt.Errorf("unknown aethercore hard constraint %q", constraint.ID)
	}
	if err := validateRoadmapText("aethercore hard constraint enforcement target", constraint.EnforcementTarget); err != nil {
		return err
	}
	if len(constraint.Modules) == 0 {
		return fmt.Errorf("aethercore hard constraint %s must name at least one module", constraint.ID)
	}
	...
	for i, moduleName := range constraint.Modules {
		if !IsRequiredCosmosSDKModule(moduleName) {
			return fmt.Errorf("aethercore hard constraint %s references unknown module %s", constraint.ID, moduleName)
		}
		if _, found := seenModules[moduleName]; found {
			return fmt.Errorf("duplicate aethercore hard constraint module %s", moduleName)
		}
		seenModules[moduleName] = struct{}{}
		if i > 0 && previousModule >= moduleName {
			return fmt.Errorf("aethercore hard constraint %s modules must be sorted canonically", constraint.ID)
		}
		previousModule = moduleName
	}
	if !constraint.ConsensusCritical {
		return fmt.Errorf("aethercore hard constraint %s must be consensus critical", constraint.ID)
	}
	if !constraint.StateTransitionGuard {
		return fmt.Errorf("aethercore hard constraint %s must guard state transitions", constraint.ID)
	}
	if !constraint.CommitmentBacked {
		return fmt.Errorf("aethercore hard constraint %s must be commitment backed", constraint.ID)
	}
	if err := validateHardConstraintEvidence(constraint.ID, constraint.Evidence); err != nil {
		return err
	}
	if constraint.ConstraintHash != "" {
		return ValidateHash("aethercore hard constraint hash", constraint.ConstraintHash)
	}
	return nil
}
...
func (constraint HardConstraint) Validate() error {
	...
	if constraint.ConstraintHash != ComputeHardConstraintHash(constraint) {
		return errors.New("aethercore hard constraint hash mismatch")
	}
	return nil
}
...
func ComputeHardConstraintHash(constraint HardConstraint) string {
	constraint = normalizeHardConstraint(constraint)
	return hashRoot("aetheris-aek-hard-constraint-v1", func(w byteWriter) {
...
func ComputeHardConstraintManifestHash(manifest HardConstraintManifest) string {
	manifest.Constraints = normalizeHardConstraints(manifest.Constraints)
	return hashRoot("aetheris-aek-hard-constraint-manifest-v1", func(w byteWriter) {
...
func validateHardConstraintEvidence(id HardConstraintID, evidence []string) error {
	if len(evidence) == 0 {
		return fmt.Errorf("aethercore hard constraint %s evidence is required", id)
	}
	seen := make(map[string]struct{}, len(evidence))
	var previous string
	for i, item := range evidence {
		if err := validateRoadmapText("aethercore hard constraint evidence", item); err != nil {
			return err
		}
		if _, found := seen[item]; found {
			return fmt.Errorf("duplicate aethercore hard constraint evidence %s", item)
		}
		seen[item] = struct{}{}
		if i > 0 && previous >= item {
			return fmt.Errorf("aethercore hard constraint %s evidence must be sorted canonically", id)
		}
		previous = item
	}
	return nil
