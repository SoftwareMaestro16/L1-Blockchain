...
func NewMixedServiceState(descriptor ServiceDescriptor, providerKey string, faultClass MixedServiceFaultClass, requiredCollateralAmount string) (MixedServiceState, error) {
	...
	if descriptor.ServiceType != ServiceTypeMixed {
		return MixedServiceState{}, errors.New("aethercore mixed state requires mixed service descriptor")
	}
...
func (state MixedServiceState) Validate() error {
	if err := validatePolicyID("aethercore mixed state service id", state.ServiceID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore mixed state provider key", state.ProviderKey); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore mixed state payment denom", state.PaymentDenom); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore mixed state escrow id", state.EscrowID); err != nil {
		return err
	}
	if err := validateAmountString("aethercore mixed state escrow amount", state.EscrowAmount); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore mixed state collateral denom", state.CollateralDenom); err != nil {
		return err
	}
	if err := validateAmountString("aethercore mixed state collateral amount", state.CollateralAmount); err != nil {
		return err
	}
	if err := validateAmountString("aethercore mixed state required collateral", state.RequiredCollateralAmount); err != nil {
		return err
	}
	if !IsMixedFaultClass(state.FaultClass) {
		return fmt.Errorf("unknown aethercore mixed fault class %q", state.FaultClass)
	}
	if err := mixedAmountAtLeast("aethercore mixed provider collateral", state.CollateralAmount, state.RequiredCollateralAmount); err != nil {
		return err
	}
	if state.ChallengeWindow == 0 {
		return errors.New("aethercore mixed service challenge window must be explicit")
	}
	if !state.FallbackDeterministic {
		return errors.New("aethercore mixed fallback path must be deterministic")
	}
	if state.FallbackServiceID != "" {
		if err := validatePolicyID("aethercore mixed fallback service id", state.FallbackServiceID); err != nil {
			return err
		}
	}
	if state.FallbackMethodID != "" {
		if err := validatePolicyID("aethercore mixed fallback method id", state.FallbackMethodID); err != nil {
			return err
		}
	}
	...
	if err := ValidateHash("aethercore mixed state hash", state.StateHash); err != nil {
		return err
	}
	if expected := ComputeMixedServiceStateHash(state); state.StateHash != expected {
		return fmt.Errorf("aethercore mixed state hash mismatch: expected %s", expected)
	}
	return nil
}
...
func AnchorMixedServiceResult(state MixedServiceState, anchor MixedResultAnchor) (MixedServiceState, MixedResultAnchor, error) {
	...
	if _, found := state.AnchorByID(anchor.AnchorID); found {
		return MixedServiceState{}, MixedResultAnchor{}, fmt.Errorf("aethercore mixed result anchor %s already exists", anchor.AnchorID)
	}
	next := state.clone()
...
func (anchor MixedResultAnchor) ValidateForState(state MixedServiceState) error {
	...
	if anchor.ServiceID != state.ServiceID {
		return errors.New("aethercore mixed result anchor service mismatch")
	}
	if anchor.ProviderKey != state.ProviderKey {
		return errors.New("aethercore mixed result anchor provider mismatch")
	}
	if anchor.ChallengeEndHeight != anchor.Height+state.ChallengeWindow {
		return errors.New("aethercore mixed result anchor challenge period mismatch")
	}
	return mixedAmountAtMost("aethercore mixed result payment", anchor.PaymentAmount, state.EscrowAmount)
}

func (anchor MixedResultAnchor) Validate() error {
	if err := ValidateHash("aethercore mixed result anchor id", anchor.AnchorID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore mixed result service id", anchor.ServiceID); err != nil {
		return err
	}
	if err := validateOptionalHash("aethercore mixed result call id", anchor.CallID); err != nil {
		return err
	}
	if err := ValidateHash("aethercore mixed request commitment", anchor.RequestCommitment); err != nil {
		return err
	}
	if err := ValidateHash("aethercore mixed result commitment", anchor.ResultCommitment); err != nil {
		return err
	}
	if err := ValidateHash("aethercore mixed receipt commitment", anchor.ReceiptCommitment); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore mixed result provider key", anchor.ProviderKey); err != nil {
		return err
	}
	if anchor.Height == 0 {
		return errors.New("aethercore mixed result anchor height must be positive")
	}
	if anchor.ChallengeEndHeight <= anchor.Height {
		return errors.New("aethercore mixed result challenge end must exceed anchor height")
	}
	if err := validateAmountString("aethercore mixed result payment amount", anchor.PaymentAmount); err != nil {
		return err
	}
	if err := ValidateHash("aethercore mixed result anchor hash", anchor.AnchorHash); err != nil {
		return err
	}
	if expected := ComputeMixedResultAnchorHash(anchor); anchor.AnchorHash != expected {
		return fmt.Errorf("aethercore mixed result anchor hash mismatch: expected %s", expected)
	}
	return nil
}
...
func OpenMixedServiceChallenge(state MixedServiceState, message MixedChallengeMessage) (MixedServiceState, MixedDispute, error) {
	...
	if !found {
		return MixedServiceState{}, MixedDispute{}, fmt.Errorf("aethercore mixed anchor %s not found", message.AnchorID)
	}
	if message.OpenedHeight == 0 || message.OpenedHeight > anchor.ChallengeEndHeight {
		return MixedServiceState{}, MixedDispute{}, errors.New("aethercore mixed challenge is outside challenge window")
	}
	...
	if _, found := state.DisputeByID(dispute.DisputeID); found {
		return MixedServiceState{}, MixedDispute{}, fmt.Errorf("aethercore mixed dispute %s already exists", dispute.DisputeID)
	}
	next := state.clone()
...
func (dispute MixedDispute) ValidateForState(state MixedServiceState) error {
	...
	if dispute.ServiceID != state.ServiceID {
		return errors.New("aethercore mixed dispute service mismatch")
	}
	if _, found := state.AnchorByID(dispute.AnchorID); !found {
		return errors.New("aethercore mixed dispute references unknown anchor")
	}
	return nil
}

func (dispute MixedDispute) Validate() error {
	if err := ValidateHash("aethercore mixed dispute id", dispute.DisputeID); err != nil {
		return err
	}
	if err := ValidateHash("aethercore mixed dispute anchor id", dispute.AnchorID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore mixed dispute service id", dispute.ServiceID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore mixed dispute challenger", dispute.Challenger); err != nil {
		return err
	}
	if err := ValidateHash("aethercore mixed challenge commitment", dispute.ChallengeCommitment); err != nil {
		return err
	}
	if dispute.OpenedHeight == 0 || dispute.ResolveByHeight <= dispute.OpenedHeight {
		return errors.New("aethercore mixed dispute challenge period is invalid")
	}
	if !IsMixedDisputeStatus(dispute.Status) {
		return fmt.Errorf("unknown aethercore mixed dispute status %q", dispute.Status)
	}
	if err := dispute.VerificationHook.Validate(); err != nil {
		return err
	}
	if err := ValidateHash("aethercore mixed dispute hash", dispute.DisputeHash); err != nil {
		return err
	}
	if expected := ComputeMixedDisputeHash(dispute); dispute.DisputeHash != expected {
		return fmt.Errorf("aethercore mixed dispute hash mismatch: expected %s", expected)
	}
	return nil
}
...
func ResolveMixedServiceChallenge(state MixedServiceState, resolution MixedDisputeResolution) (MixedServiceState, MixedSettlement, error) {
	...
	if !found {
		return MixedServiceState{}, MixedSettlement{}, fmt.Errorf("aethercore mixed dispute %s not found", resolution.DisputeID)
	}
	if dispute.Status != MixedDisputeOpen {
		return MixedServiceState{}, MixedSettlement{}, errors.New("aethercore mixed dispute is not open")
	}
	if resolution.ResolvedHeight == 0 || resolution.ResolvedHeight > dispute.ResolveByHeight {
		return MixedServiceState{}, MixedSettlement{}, errors.New("aethercore mixed dispute resolution is outside challenge window")
	}
...
func (resolution MixedDisputeResolution) Validate() error {
	if err := ValidateHash("aethercore mixed dispute resolution id", resolution.DisputeID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore mixed dispute resolver", resolution.Resolver); err != nil {
		return err
	}
	if resolution.ResolvedHeight == 0 {
		return errors.New("aethercore mixed dispute resolution height must be positive")
	}
	if resolution.ResolutionHash == "" {
		return nil
	}
	if err := ValidateHash("aethercore mixed dispute resolution hash", resolution.ResolutionHash); err != nil {
		return err
	}
	if expected := ComputeMixedDisputeResolutionHash(resolution); resolution.ResolutionHash != expected {
		return fmt.Errorf("aethercore mixed dispute resolution hash mismatch: expected %s", expected)
	}
	return nil
}
...
func SettleMixedServiceResult(state MixedServiceState, anchorID string, currentHeight uint64) (MixedServiceState, MixedSettlement, error) {
	...
	if !found {
		return MixedServiceState{}, MixedSettlement{}, fmt.Errorf("aethercore mixed anchor %s not found", anchorID)
	}
	if currentHeight <= anchor.ChallengeEndHeight {
		return MixedServiceState{}, MixedSettlement{}, errors.New("aethercore mixed settlement is still in challenge window")
	}
	if state.HasOpenDispute(anchorID) {
		return MixedServiceState{}, MixedSettlement{}, errors.New("aethercore mixed settlement has open dispute")
	}
	if state.HasSettlement(anchorID) {
		return MixedServiceState{}, MixedSettlement{}, errors.New("aethercore mixed anchor is already settled")
	}
...
func (hook MixedVerificationHook) Validate() error {
	if !IsMixedVerificationHookType(hook.HookType) {
		return fmt.Errorf("unknown aethercore mixed verification hook type %q", hook.HookType)
	}
	if hook.TargetServiceID != "" {
		if err := validatePolicyID("aethercore mixed verification target service", hook.TargetServiceID); err != nil {
			return err
		}
	}
	if hook.MethodID != "" {
		if err := validatePolicyID("aethercore mixed verification method", hook.MethodID); err != nil {
			return err
		}
	}
	if hook.ProofCommitment != "" {
		if err := ValidateHash("aethercore mixed verification proof commitment", hook.ProofCommitment); err != nil {
			return err
		}
	}
	if hook.ExpectedResultHash != "" {
		if err := ValidateHash("aethercore mixed verification expected result", hook.ExpectedResultHash); err != nil {
			return err
		}
	}
	if hook.HookType == MixedHookFallbackExecution && !hook.Deterministic {
		return errors.New("aethercore mixed fallback verification hook must be deterministic")
	}
	if hook.HookType != MixedHookFallbackExecution && hook.ProofMeterGas == 0 {
		return errors.New("aethercore mixed proof or recompute hook must be metered")
	}
	if err := ValidateHash("aethercore mixed verification hook hash", hook.HookHash); err != nil {
		return err
	}
	if expected := ComputeMixedVerificationHookHash(hook); hook.HookHash != expected {
		return fmt.Errorf("aethercore mixed verification hook hash mismatch: expected %s", expected)
	}
	return nil
}
...
func ComputeMixedResultAnchorID(anchor MixedResultAnchor) string {
	return hashParts("aetheris-aek-mixed-result-anchor-id-v1", anchor.ServiceID, anchor.CallID, anchor.RequestCommitment, anchor.ProviderKey)
}

func ComputeMixedResultAnchorHash(anchor MixedResultAnchor) string {
	return hashParts(
		"aetheris-aek-mixed-result-anchor-v1",
		anchor.AnchorID,
		anchor.ServiceID,
		anchor.CallID,
...
func ComputeMixedDisputeID(dispute MixedDispute) string {
	return hashParts("aetheris-aek-mixed-dispute-id-v1", dispute.AnchorID, dispute.Challenger, dispute.ChallengeCommitment)
}

func ComputeMixedDisputeHash(dispute MixedDispute) string {
	return hashParts(
		"aetheris-aek-mixed-dispute-v1",
		dispute.DisputeID,
		dispute.AnchorID,
		dispute.ServiceID,
		dispute.Challenger,
...
func ComputeMixedVerificationHookHash(hook MixedVerificationHook) string {
	return hashParts(
		"aetheris-aek-mixed-verification-hook-v1",
...
func ComputeMixedDisputeResolutionHash(resolution MixedDisputeResolution) string {
	return hashParts(
		"aetheris-aek-mixed-dispute-resolution-v1",
...
func ComputeMixedSettlementID(settlement MixedSettlement) string {
	return hashParts("aetheris-aek-mixed-settlement-id-v1", settlement.AnchorID, settlement.DisputeID, string(settlement.Status))
}

func ComputeMixedSettlementHash(settlement MixedSettlement) string {
	return hashParts(
		"aetheris-aek-mixed-settlement-v1",
...
func ComputeMixedServiceStateHash(state MixedServiceState) string {
	parts := []string{
		"aetheris-aek-mixed-service-state-v1",
		state.ServiceID,
		state.ProviderKey,
		state.PaymentDenom,
		state.EscrowID,
...
func validateMixedAnchors(anchors []MixedResultAnchor) error {
	...
	for _, anchor := range anchors {
		...
		if _, found := seen[anchor.AnchorID]; found {
			return fmt.Errorf("duplicate aethercore mixed result anchor %s", anchor.AnchorID)
		}
		seen[anchor.AnchorID] = struct{}{}
		if previous != "" && previous >= anchor.AnchorID {
			return errors.New("aethercore mixed result anchors must be sorted canonically")
		}
...
func validateMixedDisputes(disputes []MixedDispute) error {
	...
	for _, dispute := range disputes {
		...
		if _, found := seen[dispute.DisputeID]; found {
			return fmt.Errorf("duplicate aethercore mixed dispute %s", dispute.DisputeID)
		}
		seen[dispute.DisputeID] = struct{}{}
		if previous != "" && previous >= dispute.DisputeID {
			return errors.New("aethercore mixed disputes must be sorted canonically")
		}
...
func validateMixedSettlements(settlements []MixedSettlement) error {
	...
	for _, settlement := range settlements {
		...
		if _, found := seen[settlement.SettlementID]; found {
			return fmt.Errorf("duplicate aethercore mixed settlement %s", settlement.SettlementID)
		}
		seen[settlement.SettlementID] = struct{}{}
		if previous != "" && previous >= settlement.SettlementID {
			return errors.New("aethercore mixed settlements must be sorted canonically")
		}
...
func (settlement MixedSettlement) Validate() error {
	if err := ValidateHash("aethercore mixed settlement id", settlement.SettlementID); err != nil {
		return err
	}
	if err := ValidateHash("aethercore mixed settlement anchor id", settlement.AnchorID); err != nil {
		return err
	}
	if err := validateOptionalHash("aethercore mixed settlement dispute id", settlement.DisputeID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore mixed settlement service id", settlement.ServiceID); err != nil {
		return err
	}
	if err := validatePolicyID("aethercore mixed settlement provider key", settlement.ProviderKey); err != nil {
		return err
	}
	if !IsMixedSettlementStatus(settlement.Status) {
		return fmt.Errorf("unknown aethercore mixed settlement status %q", settlement.Status)
	}
	if settlement.SettledHeight == 0 {
		return errors.New("aethercore mixed settlement height must be positive")
	}
	if err := validatePolicyID("aethercore mixed settlement payment denom", settlement.PaymentDenom); err != nil {
		return err
	}
	if err := validateAmountString("aethercore mixed settlement payment amount", settlement.PaymentAmount); err != nil {
		return err
	}
	if !IsServicePaymentStatus(settlement.PaymentStatus) {
		return fmt.Errorf("unknown aethercore mixed settlement payment status %q", settlement.PaymentStatus)
	}
	if settlement.PenaltyDenom != "" {
		if err := validatePolicyID("aethercore mixed settlement penalty denom", settlement.PenaltyDenom); err != nil {
			return err
		}
	}
	if settlement.PenaltyAmount != "" {
		if err := validateAmountString("aethercore mixed settlement penalty amount", settlement.PenaltyAmount); err != nil {
			return err
		}
	}
	if settlement.PenaltyRecipient != "" {
		if err := validatePolicyID("aethercore mixed settlement penalty recipient", settlement.PenaltyRecipient); err != nil {
			return err
		}
	}
	if err := ValidateHash("aethercore mixed settlement hash", settlement.SettlementHash); err != nil {
		return err
	}
	if expected := ComputeMixedSettlementHash(settlement); settlement.SettlementHash != expected {
		return fmt.Errorf("aethercore mixed settlement hash mismatch: expected %s", expected)
	}
	return nil
}
