...
const (
	RootEncodingVersion     = "aetheris-aek-root-encoding-v1"
...
func QueryStateProof(state CoreState, registry ProofRegistry, proof StateCommitmentProof) (ProofVerificationResult, error) {
	...
	if !rootResponse.Found {
		return ProofVerificationResult{}, errors.New("aethercore proof query root not found")
	}
...
func VerifyStateCommitmentProof(req ProofVerificationRequest) (ProofVerificationResult, error) {
	...
	if err := ValidateHash("aethercore proof verification expected root", req.ExpectedRoot); err != nil {
		return ProofVerificationResult{}, err
	}
	...
	if proof.RootHash != req.ExpectedRoot {
		return ProofVerificationResult{}, errors.New("aethercore proof verification root mismatch")
	}
	entry, found := req.Registry.EntryFor(proof.ProofType, proof.RootType, "global")
	if !found {
		return ProofVerificationResult{}, errors.New("aethercore proof verification registry entry not found")
	}
	if !entry.Enabled {
		return ProofVerificationResult{}, errors.New("aethercore proof verification registry entry disabled")
	}
	if entry.RootHash != proof.RootHash {
		return ProofVerificationResult{}, errors.New("aethercore proof verification registry root mismatch")
	}
...
func ValidateExportImportRootChecks(state CoreState, manifest ExportManifest) error {
	...
	if !found {
		return fmt.Errorf("aethercore export/import root check missing global root at height %d", manifest.Height)
	}
	if manifest.GlobalRoot != root.GlobalRoot ||
		...
		return errors.New("aethercore export/import root set mismatch")
	}
	return nil
}
...
func (d RootEncodingDescriptor) Validate() error {
	...
	if d.Version != RootEncodingVersion {
		return errors.New("aethercore root encoding version mismatch")
	}
	if err := ValidateHash("aethercore root encoding empty root", d.EmptyRoot); err != nil {
		return err
	}
	if err := ValidateHash("aethercore root encoding hash", d.EncodingHash); err != nil {
		return err
	}
	if d.EncodingHash != ComputeRootEncodingDescriptorHash(d) {
		return errors.New("aethercore root encoding hash mismatch")
	}
	return nil
}
...
func (entry ProofRegistryEntry) ValidateFormat() error {
	...
	if !IsCommitmentProofType(entry.ProofType) {
		return fmt.Errorf("unknown aethercore proof registry proof type %q", entry.ProofType)
	}
	if err := validatePolicyID("aethercore proof registry root type", string(entry.RootType)); err != nil {
		return err
	}
	if err := validateToken("aethercore proof registry root id", entry.RootID, MaxScopeLength); err != nil {
		return err
	}
	if err := ValidateHash("aethercore proof registry root hash", entry.RootHash); err != nil {
		return err
	}
	if entry.RegistryHash != "" {
		if err := ValidateHash("aethercore proof registry entry hash", entry.RegistryHash); err != nil {
			return err
		}
	}
	return nil
...
func (entry ProofRegistryEntry) Validate() error {
	...
	if entry.RegistryHash != ComputeProofRegistryEntryHash(entry) {
		return errors.New("aethercore proof registry entry hash mismatch")
	}
	return nil
}
...
func (registry ProofRegistry) ValidateFormat() error {
	...
	if registry.Height == 0 {
		return errors.New("aethercore proof registry height must be positive")
	}
	...
	for i, entry := range registry.Entries {
		...
		if _, found := seen[key]; found {
			return fmt.Errorf("duplicate aethercore proof registry entry %s", key)
		}
		seen[key] = struct{}{}
		if i > 0 && previous >= key {
			return errors.New("aethercore proof registry entries must be ordered canonically")
		}
		previous = key
	}
	if registry.RegistryRoot != "" {
		if err := ValidateHash("aethercore proof registry root", registry.RegistryRoot); err != nil {
			return err
		}
	}
	return nil
...
func (registry ProofRegistry) Validate() error {
	...
	if registry.RegistryRoot != ComputeProofRegistryRoot(registry) {
		return errors.New("aethercore proof registry root mismatch")
	}
	return nil
}
...
func (query RootQuery) Validate() error {
	query = normalizeRootQuery(query)
	if query.Height == 0 {
		return errors.New("aethercore root query height must be positive")
	}
	if err := validatePolicyID("aethercore root query type", string(query.RootType)); err != nil {
		return err
	}
	return validateToken("aethercore root query id", query.RootID, MaxScopeLength)
}

func (response RootQueryResponse) Validate() error {
	response.QueryHash = strings.ToLower(strings.TrimSpace(response.QueryHash))
	if err := ValidateHash("aethercore root query hash", response.QueryHash); err != nil {
		return err
	}
	if !response.Found {
		return nil
...
func ComputeRootEncodingDescriptorHash(d RootEncodingDescriptor) string {
	return hashRoot("aetheris-aek-root-encoding-descriptor-v1", func(w byteWriter) {
...
func ComputeProofRegistryEntryHash(entry ProofRegistryEntry) string {
	entry = normalizeProofRegistryEntry(entry)
	return hashRoot("aetheris-aek-proof-registry-entry-v1", func(w byteWriter) {
...
func ComputeProofRegistryRoot(registry ProofRegistry) string {
	registry = normalizeProofRegistry(registry)
	return hashRoot("aetheris-aek-proof-registry-root-v1", func(w byteWriter) {
...
func ComputeRootQueryHash(query RootQuery) string {
	query = normalizeRootQuery(query)
	return hashRoot("aetheris-aek-root-query-v1", func(w byteWriter) {
...
func ComputeProofVerificationResultHash(result ProofVerificationResult) string {
	return hashRoot("aetheris-aek-proof-verification-result-v1", func(w byteWriter) {
